...
import AcDescProc from "../../features/actf_290/component/AcDescProc";
import AddAcProcM from "../../features/actf_290/page/AddAcProcM";
import EditAcProcM from "../../features/actf_290/page/EditAcProcM";
import {
  fetchAPDByID,
  fetchAllAPDMarkBByAcno,
} from "../../service/ac_proc_d/AcProcDService";
import {
  fetchAllAcProcMMarkB,
  searchAcProcMMarkBByFilter,
  addAPCM,
  editAPCM,
  fetchAPCMByID,
  activateMarkB,
  ...
  closeMarkB,
  voidAllMarkB,
  exportExcelVwProcMMarkB,
  confirmAll,
} from "../../service/ac_proc_m/AcProcMService";
import AcProcD from "../../features/actf_290/component/AcProcD";
import { fetchViewVApA } from "../../service/vw_apdue_all/vwApdueAll";
import ModifyLationPopup from "../../features/actf_290/page/ModifyLationPopup";
import ImportLinkPopup from "../../features/actf_290/page/ImportLinkPopup";
...
const Actf290 = () => {
  ...
  const fetchAll = async (authData = null, pageSize = 5, offset = 0) => {
    ...
    setData([{ tableName: "AC_PROC_M_1", data: combinedData[0].data }]);
  ...
  const fetchAllTranslations = async () => {
    try {
      ...
      const controls = await fetchTableControlTranslations("ACTF_290");
  ...
  const fetchAPDByAcno = async (
    ...
    try {
      if (!selectRows || selectRows.length === 0) {
        setAPDData([{ tableName: "AC_PROC_D_1", data: [] }]);
        setSelectAPD([]);
        return;
      }
      ...
      const allow = Array.isArray(authorization)
        ...
        : null;
      const response = await fetchAllAPDMarkBByAcno(
      ...
      if (response && response.data) {
        ...
        setAPDData([{ tableName: "AC_PROC_D_1", data: childrenData }]);
      ...
      console.error(" Error fetching AC_PROC_D_1 by ac_no:", error);
  ...
  const fetchADPByAcno = async (
    ...
    try {
      if (!selectRows || selectRows.length === 0) {
        setADPData([{ tableName: "VW_APDUE_ALL", data: [] }]);
        setSelectADP([]);
        return;
      }
      const offset = shouldResetPagination
      ...
      const response = await fetchViewVApA(
        selectRows[0]?.vend_no,
        selectRows[0]?.com_invoice,
        selectRows[0]?.col6?.substr(0, 1),
        selectRows[0]?.col4,
        language,
        pageSize,
        offset,
      ...
      if (response && response.data) {
        ...
        setADPData([{ tableName: "VW_APDUE_ALL", data: childrenData }]);
        if (response.total !== undefined && response.total !== null) {
          setTotalADPData(response.total);
        }
        if (childrenData.length > 0) {
      ...
      console.error(" Error fetching VW_APDUE_ALL by ac_no:", error);
      showErrorToast(getControlLabel, "noti_fail_add_1", `Failed to add`);
    }
  };
  ...
  const refreshCurrentData = async () => {
    ...
    if (isSearch && searchFilter) {
      ...
      const response = await searchAcProcMMarkBByFilter(
        searchFilter,
        user.factory,
        user.department,
        user.user_code,
      ...
      const responseData = await fetchAllAcProcMMarkB(
        user.factory,
        user.department,
        user.user_code,
        allow || "1",
      ...
      if (responseData && responseData.data) {
        setData([{ tableName: "AC_PROC_M_1", data: responseData.data }]);
    ...
    try {
      if (selectRows.length === 1) {
        ...
        if (record?.locked_information === user?.clientInfo) {
          ...
          const { d_type_name, stoc_type_name, ...finalLock } = unlockData;
          await editAPCM(
            user?.access_token,
            user?.factory,
            user?.department,
  ...
  const handleEditAPCM = async (updateRow, title, skipTimestamp = false) => {
    try {
      const { statusText, d_type_name, stoc_type_name, ...cleanData } =
        updateRow;
      if (!skipTimestamp) {
    ...
    try {
      ...
      if (response && response.data) {
        if (response.data.length === 0) {
          ...
          setHasMore(false);
          setTotalData(0);
          setData([{ tableName: "AC_PROC_M_1", data: response.data }]);
        } else {
          setSelectRows([response.data[0]]);
  ...
  const handleClose = async () => {
    try {
      await handleStatusChange("close", [1, 7], async () => {
        await closeMarkB(user?.factory, selectRows[0]?.ac_no, user?.user_code);
      });
    } catch (error) {
  ...
  const handleExport = async () => {
    ...
    const excel = await exportExcelVwProcMMarkB(
      user?.factory,
      user?.department,
      user?.user_code,
      allow || "1",
  ...
  const handleOpenImportLink = async () => {
    ...
    const response = await fetchAllAcProcMMarkB(
      user?.factory,
      user?.department,
      user?.user_code,
      allow || "1",
    ...
    const isMoreThan6 = response?.data?.length > 0;
    setTotalImportData(isMoreThan6 ? 7 : 0);
  ...
  return (
    <>
      <Container maxWidth="xxl">
        <Stack direction="row" flexWrap="wrap" sx={{ rowGap: 1 }}>
          ...
          >
            ...
            >
              {/* Bảng chính và AC_PROC_D_1 - chiếm 60% */}
              <Box
                sx={{
                  flex: "0 0 60%",
                  minWidth: 0,
      ...
      <ImportLinkPopup
        ...
        getControlLabel={getControlLabel}
        isSmall={totalImportData < 6}
      />
    </>
  );
};
export default Actf290;
