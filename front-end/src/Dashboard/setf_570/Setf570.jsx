...
import SeShippingD from "../../features/setf_570/component/SeShippingD";
import {
  addSeShippingM,
  confirmAll,
  editSeShhippingM,
...
const Setf570 = () => {
  ...
  const fetchDataByInmNo = async (
    ...
    const response = await fetchAllSeShipingD(
      user?.access_token,
      user?.factory,
      selectRows[0]?.cust_id,
      selectRows[0]?.si_seq,
    ...
    if (response && response.data) {
      ...
      setSSDData([{ tableName: "SE_SHIPPING_D", data: childrenData }]);
  ...
  const fetchAllTranslations = async () => {
    try {
      ...
      const controls = await fetchTableControlTranslations("SETF_570");
  ...
  const refreshCurrentData = async () => {
    ...
    if (isSearch && searchFilter) {
      ...
      let response;
      response = await searchSeShippingMByFilter(
        searchFilter,
        user.factory,
        user.department,
        user.user_code,
      ...
      if (response && response.data) {
        setData([{ tableName: "SE_SHIPING_M", data: response.data }]);
      ...
      const responseData = await fetchAllShippingM(
        user.factory,
        user.department,
        user.user_code,
        allow || "1",
      ...
      if (responseData && responseData.data) {
        setData([{ tableName: "SE_SHIPING_M", data: responseData.data }]);
  ...
  const handleUpdateConfirm = async () => {
    ...
    try {
      ...
      if (response && response.data) {
        setSSDData([{ tableName: "SE_SHIPPING_D", data: response.data }]);
  ...
  const handleSearchMaster = async (newFilter, pageSize = 10, offset = 0) => {
    try {
      ...
      const response = await searchSeShippingMByFilter(
        newFilter,
        user.factory,
        user.department,
        user.user_code,
      ...
      if (response && response.data && response.data.length > 0) {
        ...
        setIsSearch(true);
        setSearchData([]);
        setData([{ tableName: "SE_SHIPING_M", data: [] }]);
        setSelectRows([]);
        setSSDData([{ tableName: "SE_SHIPPING_D", data: [] }]);
        setSelectSSD([]);
        setTotalData(0);
        setTotalSSDData(0);
        setCurrentOffset(0);
  ...
  const handleConfirm = async () => {
    await handleStatusChange(7, "confirm", [1], async (freshRecord) => {
      ...
      try {
        ...
        await refreshCurrentData();
        await fetchDataByInmNo(false, null, null);
      } catch (error) {
