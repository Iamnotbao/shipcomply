...
import { exportPDFVwAcShoeBom } from "../../service/vw_ac_shoebom/ViewAcShoeBomService";
import {
  exportExcel,
  fetchViewAcSum,
  searchVwAcSumByFilter,
} from "../../service/vw_ac_sum/vwAcSum";
...
const Actf310 = () => {
  ...
  const fetchAll = async (authData = null, pageSize = 10, offset = 0) => {
    ...
    setData([{ tableName: "VW_AC_SUM", data: combinedData[0].data }]);
  ...
  const handleSearchByFilter = async (
    ...
    try {
      ...
      const response = await searchVwAcSumByFilter(
        filteredShoe,
        user.factory,
        user.department,
        user.user_code,
      ...
      if (response && response.data) {
        if (response.data.length === 0) {
          ...
          setData([{ tableName: "VW_AC_SUM", data: response.data }]);
        }
      }
    } catch (error) {
  ...
  const handlePDF = async () => {
    ...
    try {
      ...
      showErrorToast(getControlLabel, `noti_fail_excel`, error?.message);
    }
  };
  ...
  const handlePageChange = async (newPage, newPageSize) => {
    ...
    //  CASE 3: Không search
    const responseData = await fetchViewAcSum(
      user?.factory,
      user?.department,
      user?.user_code,
