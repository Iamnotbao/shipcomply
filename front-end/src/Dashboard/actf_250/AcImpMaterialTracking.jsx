...
import AddAcImpMaterialTrackingPage from "../../features/ac_imp_material_tracking/page/AddAcImpMaterialTrackingPage";
import EditAcImpMaterialTrackingPage from "../../features/ac_imp_material_tracking/page/EditAcImpMaterialTrackingPage";
import {
  showErrorToast,
  showSuccessToast,
  showWarningToast,
...
const AcImpMaterialTracking = () => {
  ...
  const fetchAll = async (authData = null, pageSize = 10, offset = 0) => {
    ...
    setData([
      { tableName: "AC_IMP_MATERIAL_TRACKING", data: combinedData[0].data },
    ]);
  ...
  const fetchAllTranslations = async () => {
    try {
      ...
      const controls = await fetchTableControlTranslations("ACTF_250");
  ...
  const fetchRecordFromDB = async (record) => {
    try {
      const response = await fetchIMTByID(
  ...
  const refreshCurrentData = async () => {
    ...
    if (isSearch && searchFilter) {
      ...
      if (response && response.data) {
        ...
        if (currentSelection) {
          const currentKey = `${currentSelection.factory_code}-${currentSelection.invoice_no}-${currentSelection.sort}`;
          const foundRecord = response.data.find(
            (item) =>
              `${item.factory_code}-${item.invoice_no}-${item.sort}` ===
              currentKey,
          );
      ...
      const responseData = await fetchIMT(
        user.factory,
        user.department,
        user.user_code,
        allow || "1",
      ...
      if (responseData && responseData.data) {
        setData([
          { tableName: "AC_IMP_MATERIAL_TRACKING", data: responseData.data },
        ]);
    ...
    try {
      if (selectRows.length === 1) {
        ...
        if (record?.locked_information === user?.clientInfo) {
          ...
          const { declaration_category_name, ...cleanData } = unlockData;
          await editIMT(
            user?.factory,
            user?.department,
            user?.user_code,
          ...
          //  Dùng refreshCurrentData - giữ nguyên vị trí
  ...
  const handleOpenEdit = async () => {
    try {
      ...
      await handleEditITM(lockData, "", true);
    ...
    try {
      ...
      if (response.success) {
        ...
        showSuccessToast(
          getControlLabel,
          "noti_success_add",
          `Add successfully with id ${response.invoice_no}!`,
        );
        setData([
          {
  ...
  const handleEditITM = async (updateRow, title, skipTimestamp = false) => {
    try {
      const { statusText, declaration_category_name, ...cleanData } = updateRow;
      if (!skipTimestamp) {
        cleanData.last_user = user.user_code;
        cleanData.last_date = new Date().toISOString();
      }
      const result = await editIMT(
        user?.factory,
        user?.department,
        user?.user_code,
        authorization?.find((item) => item.field === "query_level")?.title,
          ...
          if (resultPage !== currentPage) {
            ...
            if (responseData && responseData.data) {
              ...
              const editedRecord = responseData.data.find(
                (item) =>
                  `${item.factory_code}-${item.invoice_no}-${item.sort}` ===
                  `${cleanData.factory_code}-${cleanData.invoice_no}-${cleanData.sort}`,
              );
    ...
    try {
      ...
      if (response && response.data) {
        if (response.data.length === 0) {
          ...
          setData([{ tableName: "VW_AC_SHOEBOM", data: response.data }]);
        }
      }
    } catch (error) {
  ...
  const handleStatusChange = async (
    ...
    try {
      ...
      if (user.user_code !== "admin") {
        ...
        const allowS = authorization?.find(
      ...
      if (
        ...
        //  Dùng refreshCurrentData thay vì fetchAll hoặc handleSearchByFilter
        await refreshCurrentData();
        return;
      }
  ...
  const handleImportLink = () => {
    window.open(`${import.meta.env.VITE_LINK_IMP_URL}/actr_250`, "_blank");
  };
  //==========END HANDLER SECTION ================
  ...
  const getFetchData = {
    declaration_category: createBasicCallback("CDC"),
    loading_way: createBasicCallback("2118"),
  };
  return (
    <>
      {/* ========== MAIN CONTENT AREA ========== */}
...
export default AcImpMaterialTracking;
