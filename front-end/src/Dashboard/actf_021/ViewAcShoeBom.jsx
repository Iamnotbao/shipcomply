...
const ViewAcShoeBom = () => {
  ...
  const fetchAll = async (authData = null, pageSize = 10, offset = 0) => {
    ...
    setData([{ tableName: "VW_AC_SHOEBOM", data: combinedData[0].data }]);
  ...
  const fetchAllTranslations = async () => {
    try {
      const controls = await fetchTableControlTranslations("ACTF_021");
  ...
  const handlePageChange = async (newPage, newPageSize) => {
    ...
    //  CASE 3: Không search
    const responseData = await fetchViewAcShoeBom(
      user?.factory,
      user?.department,
      user?.user_code,
