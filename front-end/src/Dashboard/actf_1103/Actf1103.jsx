...
const Actf1103 = () => {
  ...
  const fetchAll = async (authData = null) => {
    ...
    const combinedData = await fnQuery([
      () => fetchAllVwContUse(user.factory, selectedContImp, language),
    ]);
  ...
  // ========== USEEFFECT SECTION ==========
  //  ĐÚNG - Parse JSON và handle null
  useEffect(() => {
    const storedData = localStorage.getItem("selectAcContD");
    if (storedData) {
      try {
