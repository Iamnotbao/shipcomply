...
import AcContD from "../../features/actf_120/component/AcContD";
import {
  addAcContM,
  confirmAll,
  editAcContM,
...
import {
  fetchViewContExpSetting,
  searchVwAcContExpByFilter,
  copyContract,
...
import AddAcContM from "../../features/actf_120/page/AddAcContM";
import EditAcContM from "../../features/actf_120/page/EditAcContM";
import VwChgExmp from "../../features/actf_120/component/VwChgExmp";
import { fetchAllVwChgExmp } from "../../service/vw_chg_exmp/VwChgExmp";
import DetailAcContM from "../../features/actf_120/page/DetailAcContM";
const Actf120 = () => {
  ...
  const fetchDataByACD = async (
    ...
    const response = await fetchAllVwChgExmp(
      user?.access_token,
      user.factory,
      selectAcContD[0],
      language,
    ...
    if (response && response.data) {
      let childrenData = response.data;
      setVwContUseData([{ tableName: "VW_CHG_EXMP", data: childrenData }]);
      // if (response.total !== undefined && response.total !== null) {
  ...
  const refreshCurrentAcItemM = async (
    ...
    if (isSearch && searchFilter) {
      ...
      let response;
      response = await searchVwAcContExpByFilter(
        searchFilter,
        user.factory,
        user.department,
        user.user_code,
      ...
      if (response && response.data) {
        setData([{ tableName: "VW_CONT_EXP", data: response.data }]);
        if (currentSelection) {
      ...
      const responseData = await fetchViewContExpSetting(
        user.factory,
        user.department,
        user.user_code,
        allow || "1",
      ...
      if (responseData && responseData.data) {
        setData([{ tableName: "VW_CONT_EXP", data: responseData.data }]);
  ...
  useEffect(() => {
    if (
      ...
      setVwContUseData([{ tableName: "VW_CHG_EXMP", data: [] }]);
      setSelectVwContUse([]);
    }
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const allData = await fetchViewContExpSetting(
          user?.factory,
          user?.department,
          user?.user_code,
          allow,
        ...
        setData([{ tableName: "VW_CONT_EXP", data: allData.data }]);
  ...
  const handleOk = async (e) => {
    ...
    try {
      ...
      if (response && response.success) {
        ...
        setData([{ tableName: "VW_CONT_EXP", data: allData.data }]);
        const findData = allData.data.find((item) => item.cont_no === cont_no);
      ...
      if (result.success) {
        ...
        if (isSearch) {
          ...
          if (resultPage !== currentACMPage) {
            ...
            if (responseData && responseData.data) {
              setData([{ tableName: "AC_CONT_M", data: responseData.data }]);
              setHasMore(responseData?.hasMore);
  ...
  const handleSearchMaster = async (newFilter, pageSize = 5, offset = 0) => {
    try {
      ...
      const response = await searchVwAcContExpByFilter(
        newFilter,
        user.factory,
        user.department,
        user.user_code,
      ...
      if (response && response.data && response.data.length > 0) {
        ...
        if (detailsResponse && detailsResponse.data) {
          ...
          if (detailsResponse.data.length > 0) {
            ...
            const vwResponse = await fetchAllVwChgExmp(
              user?.access_token,
              user.factory,
        ...
        setIsSearch(true);
        setSearchData([]);
        setData([{ tableName: "VW_CONT_EXP", data: [] }]);
        setSelectRows([]);
        setAcContDData([{ tableName: "AC_CONT_D", data: [] }]);
        setSelectAcContD([]);
        setTotalData(0);
