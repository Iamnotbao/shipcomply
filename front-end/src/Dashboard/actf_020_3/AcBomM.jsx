...
const AcBomM = () => {
  ...
  const fetchAll = async (authData = null, pageSize = 10, offset = 0) => {
    ...
    setData([{ tableName: "AC_BOM_M", data: combinedData[0].data }]);
  ...
  const fetchRecordFromDB = async (record) => {
    try {
      const response = await fetchAcBomMByID(
        selectRows[0]?.factory_code,
        selectRows[0]?.prod_acno,
  ...
  const refreshCurrentData = async () => {
    ...
    if (isSearch && searchFilter) {
      ...
      if (response && response.data) {
        ...
        if (currentSelection) {
          const currentKey = `${currentSelection.factory_code}-${currentSelection.prod_acno}-${currentSelection.item_acno}`;
          const foundRecord = response.data.find(
            (item) =>
              `${item.factory_code}-${item.prod_acno}-${item.item_acno}` ===
              currentKey,
          );
      ...
      const responseData = await fetchAcBomM(
        user.factory,
        user.department,
        user.user_code,
        allow || "1",
      ...
      if (responseData && responseData.data) {
        setData([{ tableName: "AC_BOM_M", data: responseData.data }]);
    ...
    try {
      ...
      if (response.success) {
        ...
        showSuccessToast(
          getControlLabel,
          "noti_success_add",
          `Add successfully with id ${data.prod_acno}-${data.item_acno}!`,
        );
        setHasMore(responseData?.hasMore);
          ...
          if (resultPage !== currentPage) {
            ...
            if (responseData && responseData.data) {
              ...
              const editedRecord = responseData.data.find(
                (item) =>
                  `${item.factory_code}-${item.prod_acno}-${item.item_acno}` ===
                  `${cleanData.factory_code}-${cleanData.prod_acno}-${cleanData.item_acno}`,
              );
    ...
    try {
      ...
      if (response && response.data) {
        if (response.data.length === 0) {
          ...
          setHasMore(false);
          setTotalData(0);
          setData([{ tableName: "AC_BOM_M", data: response.data }]);
        } else {
          setSelectRows([response.data[0]]);
          // setJumpToRow(response.data[0]);
  ...
  const handleStatusChange = async (
    ...
    try {
     ...
     freshRecord.locked_information !==user?.clientInfo
      ...
      const response = await editABomM(
        user?.factory,
        user?.department,
        user?.user_code,
        authorization?.find((item) => item.field === "query_level")?.title,
