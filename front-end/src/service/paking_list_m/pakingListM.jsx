...
const API = "/paking_list_m";
export const fetchAllListM= async (
  factory_code,
  department_code,
  user_code,
  query_level,
...
export const fetchPackingListMByID = async (factory_code, cust_id, si_seq) => {
  const response = await axios.get(
...
export const searchPakingListMByFilter = async (
  filtered,
  factory_code,
  department_code,
  user_code,
...
export const pdfToPS = async (
  ...
  try {
    const response = await axios.get(
      `${API}/pdf_paking_list?factory_code=${factory_code}&department_code=${department_code}&user_code=${user_code}&query_level=${query_level}`,{
        headers: {
          authorization: `Bearer ${token}`,
        },
        responseType: "blob",
    ...
    const link = document.createElement("a");
    link.href = url;
    link.setAttribute("download", "packing_list_1.pdf");
    document.body.appendChild(link);
    link.click();
    link.remove();
    return response.data ? response.data : "";
