...
import GenegrateGoodsCode from "../page/GenerateGoodsCode";
import PlanShipDate from "../page/PlanShipDate";
const AcChgD = ({
  data,
  setData,
  setSelectAcChgD,
 ...
 const refreshCurrentData = async (updatedRecord = null) => {
          if (updatedRecord) {
            const newData = await fetchAllAcChgDExpByAcno(
              user?.factory,
              selectRows[0]?.ac_no,
              user?.department,
              user?.user_code,
            ...
            setData([{ tableName: "AC_CHG_D", data: newData?.data }]);
  ...
  useEffect(() => {
    const handleDataFetch = async () => {
      if (
        ...
        setData([{ tableName: "AC_CHG_D", data: [] }]);
        setSelectAcChgD([]);
        return;
      }
  ...
  const handleEditClose = async (chooseRow) => {
    try {
      if (selectAcChgD.length === 1) {
        const record = chooseRow || selectAcChgD[0];
  ...
  const handleStatusChange = async (
    ...
    if (selectAcChgD.length !== 1) return;
  ...
  const handleAutoAdd = async () => {
    try {
      ...
      setSelectAcChgD([response?.data[0]] || []);
    } catch (error) {
  ...
  const handleAdd = async (newData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const responseData = await fetchAllAcChgDExpByAcno(
          selectRows[0]?.factory_code,
          selectRows[0]?.ac_no,
          user.department,
          user.user_code,
        ...
        if (responseData && responseData.data) {
          ...
          setData([{ tableName: "AC_CHG_D", data: responseData.data }]);
          setHasMore(responseData?.hasMore);
          setSelectAcChgD([response.data]);
  ...
  return (
    <>
      <Box>
       ...
       {/*       {getControlLabel("ttl_table_d_1", "AC_CHG_D")} */}
        </Typography>
        <Stack
          direction="row"
          flexWrap="wrap"
        ...
        >
          <div style={{ minWidth: 0, width: "100%", maxWidth: "100%" }}>
            <DataTable
              ...
              onGenerateGC={onOpenGenegrateGC}
              type="2"
              onPlanDate={onOpenPD}
      ...
      <EditAcChgD
        ...
        getControlLabel={getControlLabel}
        getColumnLabel={getColumnLabel}
        selectRows={selectAcChgD.length > 0 ? selectAcChgD : {}}
        user={user}
      />
      <GenegrateGoodsCode
        openLink={openGenegrateGC}
        onClose={onCloseGenegrateGC}
        onSave={onGenerateGC}
        getControlLabel={getControlLabel}
        getColumnLabel={getColumnLabel}
        language={language}
      ...
      <PlanShipDate
        ...
        data={pDData}
        setData={setPDData}
        jumpToRow={jumpToRowPD}
        setJumpToRow={setJumpToRowPD}
        selectRows={selectRows.length > 0 ? selectRows : []}
        selectSOMC={selectPD}
        setSelectSOMC={setSelectPD}
        subAuthentication={subAuthentication}
        currentPage={currentPDPage}
        currentPageSize={currentPDPageSize}
        setCurrentOffset={setCurrentPDOffset}
        totalData={totalPDData}
        setTotalData={setTotalPDData}
        hasMore={hasPDMore}
        setHasMore={setHasPDMore}
        isSearch={isPDSearch}
        setIsSearch={setIsPDSearch}
        searchFilter={searchPDFilter}
        setSearchFilter={setSearchPDFilter}
        setCurrentPage={setCurrentPDPage}
        setCurrentPageSize={setCurrentPDPageSize}
        onPageChange={handlePDPageChange}
        user={user}
        selectCheckSOMC={selectCheckPD}
