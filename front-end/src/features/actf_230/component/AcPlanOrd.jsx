...
import DeletePlanOrd from "../page/DeletePlanOrd";
const AcPlanOrd = ({
  data,
  setData,
  setSelectAcPlanOrd,
  ...
  const refreshCurrentData = async (updatedRecord = null) => {
    if (updatedRecord) {
      ...
      setSelectAcPlanOrd([updatedRecord]);
      return;
    }
  };
  useEffect(() => {
  ...
  useEffect(() => {
    const handleDataFetch = async () => {
      if (
        ...
        setData([{ tableName: "AC_PLAN_ORD", data: [] }]);
        setSelectAcPlanOrd([]);
        return;
      }
  ...
  const handleEditClose = async (chooseRow) => {
    try {
      if (selectAcPlanOrd.length === 1) {
        const record = chooseRow || selectAcPlanOrd[0];
  ...
  const handleOpenEdit = async () => {
    try {
      ...
      const record = selectAcPlanOrd[0];
  ...
  const handleStatusChange = async (
    ...
    if (selectAcPlanOrd.length !== 1) return;
    ...
    try {
      ...
      if (
        ...
        setSelectAcPlanOrd([freshRecord]);
    ...
    try {
      ...
      if (response.success) {
        ...
        if (responseData && responseData.data) {
          ...
          setData([{ tableName: "AC_PLAN_ORD", data: responseData.data }]);
          setSelectAcPlanOrd([response.data]);
      ...
      if (isSearch) {
        ...
        if (resultPage !== currentPage) {
          ...
          if (responseData && responseData.data) {
            ...
            if (editedRecord) {
              setSelectAcPlanOrd([editedRecord]);
    ...
    <>
      <Box>
        <Typography variant="h5" textAlign={"center"} fontWeight={"bold"}>
          {/*      {getControlLabel("ttl_table_d_2", "AC_PLAN_ORD")} */}
        </Typography>
        <Stack
          direction="row"
          flexWrap="wrap"
      ...
      <DeletePlanOrd
        ...
        getColumnLabel={getColumnLabel}
        getControlLabel={getControlLabel}
        selectRows={selectAcPlanOrd.length > 0 ? selectAcPlanOrd : []}
      />
    </>
  );
};
export default AcPlanOrd;
