...
const AcPlanPack = ({
  ...
  const refreshCurrentData = async (updatedRecord = null) => {
    if (updatedRecord) {
      ...
      setSelectAcPlanPack([updatedRecord]);
      return;
    }
  };
  useEffect(() => {
  ...
  useEffect(() => {
    const handleDataFetch = async () => {
      if (
        ...
        setData([{ tableName: "AC_PLAN_PACK", data: [] }]);
        setSelectAcPlanPack([]);
        return;
      }
  ...
  const handleEditClose = async (chooseRow) => {
    try {
      if (selectAcPlanPack.length === 1) {
        const record = chooseRow || selectAcPlanPack[0];
  ...
  const handleOpenEdit = async () => {
    try {
      ...
      const record = selectAcPlanPack[0];
  ...
  const handleStatusChange = async (
    ...
    if (selectAcPlanPack.length !== 1) return;
    ...
    try {
      ...
      if (
        ...
        setSelectAcPlanPack([freshRecord]);
      ...
      if (response.success) {
        ...
        setSelectAcPlanPack([updateData]);
    ...
    try {
      ...
      if (response.success) {
        ...
        if (responseData && responseData.data) {
          ...
          setData([{ tableName: "AC_PLAN_PACK", data: responseData.data }]);
          setSelectAcPlanPack([response.data]);
      ...
      if (isSearch) {
        ...
        if (resultPage !== currentPage) {
          ...
          if (responseData && responseData.data) {
            ...
            if (editedRecord) {
              setSelectAcPlanPack([editedRecord]);
    ...
    <>
      <Box>
        <Typography variant="h5" textAlign={"center"} fontWeight={"bold"}>
          {/*      {getControlLabel("ttl_table_d_2", "AC_PLAN_PACK")} */}
        </Typography>
        <Stack
          direction="row"
          flexWrap="wrap"
