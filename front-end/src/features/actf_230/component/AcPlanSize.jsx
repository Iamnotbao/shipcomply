...
import GenegrateProdMPopup from "../page/EditProdMPopup";
const AcPlanSize = ({
  data,
  setData,
  setSelectAcPlanSize,
  ...
  const refreshCurrentData = async (updatedRecord = null) => {
    if (updatedRecord) {
      ...
      setSelectAcPlanSize([updatedRecord]);
      return;
    }
  };
  useEffect(() => {
  ...
  useEffect(() => {
    const handleDataFetch = async () => {
      if (
        ...
        setData([{ tableName: "AC_PLAN_SIZE", data: [] }]);
        setSelectAcPlanSize([]);
        return;
      }
  ...
  const handleEditClose = async (chooseRow) => {
    try {
      if (selectAcPlanSize.length === 1) {
        const record = chooseRow || selectAcPlanSize[0];
  ...
  const handleOpenEdit = async () => {
    try {
      ...
      const record = selectAcPlanSize[0];
  ...
  const handleStatusChange = async (
    ...
    if (selectAcPlanSize.length !== 1) return;
    ...
    try {
      ...
      if (
        ...
        setSelectAcPlanSize([freshRecord]);
      ...
      if (response.success) {
        ...
        setSelectAcPlanSize([updateData]);
    ...
    try {
      ...
      if (response.success) {
        ...
        if (responseData && responseData.data) {
          ...
          setData([{ tableName: "AC_PLAN_SIZE", data: responseData.data }]);
          setSelectAcPlanSize([response.data]);
    ...
    <>
      <Box>
        <Typography variant="h5" textAlign={"center"} fontWeight={"bold"}>
          {/*      {getControlLabel("ttl_table_d_2", "AC_PLAN_SIZE")} */}
        </Typography>
        <Stack
          direction="row"
          flexWrap="wrap"
        ...
        >
          <div style={{ minWidth: 0, width: "100%", maxWidth: "100%" }}>
            <DataTable
              ...
              onGeneratePM={onOpenGenegrateGC}
            />
          </div>
        </Stack>
      </Box>
      <GenegrateProdMPopup
        openLink={openGenegratePM}
        onClose={onCloseGenegratePM}
        onSaveAll={onGeneratePM}
        onSaveOnly={handleEditAPS}
        getControlLabel={getControlLabel}
        getColumnLabel={getColumnLabel}
        language={language}
        user={user}
        selectRows={selectAcPlanSize.length ? selectAcPlanSize : []}
      />
    </>
  );
};
export default AcPlanSize;
