...
const AcChgD = ({
  ...
  const refreshCurrentData = async (updatedRecord = null) => {
          if (updatedRecord) {
            const newData = await fetchAllAcChgDByAcno(
              user?.factory,
              selectRows[0]?.ac_no,
              user?.department,
              user?.user_code,
  ...
  const handleAutoAdd = async () => {
    ...
    if(selectRows[0]?.status !== 1) {
      showErrorToast(getControlLabel, "noti_fail_parent_status", "Please change the master row to new!");
      return;
    }
    try {
      ...
      showSuccessToast(getControlLabel, "noti_success_auto_add");
    } catch (error) {
  ...
  const handleAdd = async (newData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const responseData = await fetchAllAcChgDByAcno(
          selectRows[0]?.factory_code,
          selectRows[0]?.ac_no,
          user.department,
          user.user_code,
