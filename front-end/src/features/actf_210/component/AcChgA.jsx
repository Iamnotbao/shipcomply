...
import AddAcChgA from "../page/AddAcChgA";
import EditAcChgA from "../page/EditAcChgA";
import {
  addAcChgA,
  editAcChgA,
...
const AcChgA = ({
  ...
  const refreshCurrentData = async (updatedRecord = null) => {
    if (updatedRecord) {
      const newData = await fetchAllAcChgAByAcno(
        user?.factory,
        selectRows[0]?.ac_no,
        user?.department,
        user?.user_code,
      ...
      setData([{ tableName: "AC_CHG_A", data: newData?.data }]);
  ...
  useEffect(() => {
    const handleDataFetch = async () => {
      if (
        ...
        setData([{ tableName: "AC_CHG_A", data: [] }]);
        setSelectAcChgA([]);
        return;
      }
  ...
  const handleAdd = async (newData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const responseData = await fetchAllAcChgAByAcno(
          selectRows[0]?.factory_code,
          selectRows[0]?.ac_no,
          user.department,
          user.user_code,
        ...
        if (responseData && responseData.data) {
          ...
          setData([{ tableName: "AC_CHG_A", data: responseData.data }]);
