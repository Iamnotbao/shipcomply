...
import ExchangeRatePopup from "../page/ExchangeRatePopup";
const AcProcD = ({
  data,
  setData,
  setSelectAPD,
  ...
  const refreshCurrentData = async (updatedRecord = null) => {
    if (updatedRecord) {
      const newData = await fetchAllAPDMarkBByAcno(
        user?.factory,
        selectRows[0]?.ac_no,
        user?.department,
        user?.user_code,
  ...
  useEffect(() => {
    const handleDataFetch = async () => {
      if (
        ...
        setData([{ tableName: "AC_PROC_D_1", data: [] }]);
        setSelectAPD([]);
        return;
      }
  ...
  const handleAutoAdd = async () => {
    ...
    try {
      const response = await autoAddAcProcDMarkB(
        user?.access_token,
        user?.factory,
        user?.department,
        user?.user_code,
  ...
  const handleOpenExchangeRate = () => {
    ...
    if(selectRows[0]?.status !== 1){
      showErrorToast(
        getControlLabel,
        "noti_fail_parent_status",
        "Please Unconfirm the master first!",
      );
      return;
    } ;
  ...
  const handleExchangeRateB = async (e) => {
    try {
      e.preventDefault();
      const formData = new FormData(e.target);
      const data = Object.fromEntries(formData.entries());
      const response = await exChangeRateAcProcDMarkB(
        user?.factory,
        selectRows[0]?.ac_no,
        data?.in_crate,
        language,
      ...
      showErrorToast(getControlLabel, "noti_fail_exchange_rate", error);
    }
  };
  const handleAdd = async (newData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const responseData = await fetchAllAPDMarkBByAcno(
          selectRows[0]?.factory_code,
          selectRows[0]?.ac_no,
          user.department,
          user.user_code,
        ...
        if (responseData && responseData.data) {
          ...
          setData([{ tableName: "AC_PROC_D_1", data: responseData.data }]);
  ...
  return (
    <>
          ...
          <div style={{ minWidth: 0, width: "100%", maxWidth: "100%" }}>
           ...
           {/*      {getControlLabel("ttl_table_d_1", "AC_PROC_D")} */}
            ...
            <DataTable
              ...
              onExchangeRate={handleOpenExchangeRate}
              hasMore={hasMore}
            />
          </div>
        </Stack>
      ...
      />
      <ExchangeRatePopup
        openLink={openExchangeRate}
        onClose={handleCloseExchangeRate}
        onExchangeRate={handleExchangeRateB}
        getControlLabel={getControlLabel}
        selectRow={selectRows.length > 0 ? selectRows[0] : null}
      />
    </>
  );
};
