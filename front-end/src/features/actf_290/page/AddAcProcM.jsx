...
import { createAcno } from "../../../service/ac_proc_m/AcProcMService";
import {
  fetchFieldDropdown,
  fetchInContDropdown,
...
const AddAcProcM = ({
  ...
  const loadStocType = async (dtype) => {
    try {
      ...
      console.log("adudu ", stocTypeResult?.data[0]?.stoc_type);
          ...
          <Controller
            ...
            render={({ field }) => (
              <Dropdown
                ...
                pageSize={10}
                disabled={!vend_no}
                helperText={!vend_no ? "Please select AC Item No first" : ""}
                {...extraProps}
              />
            )}
          />
