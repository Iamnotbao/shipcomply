...
const EditAcContD = ({
  ...
  const cont_money = watch("cont_money");
  const goods_code = watch("goods_code");

  useEffect(() => {
    const qty = parseFloat(cont_qty) || 1;
    const money = parseFloat(cont_money) || 0;
    const price = money/qty;
    setValue("cont_price", price.toFixed(8));
  }, [cont_qty, cont_money, setValue]);
  useEffect(() => {
    if (basicData && Object.keys(basicData).length > 0) {
      ...
      setDropdownValues({
        goods_code: basicData.goods_code || "",
        unit: basicData.unit || "",
      });
    }
  }, [basicData, reset]);
  ...
  useEffect(() => {
    if (goods_code !== basicData?.goods_code && goods_code !== dropdownValues.goods_code) {
      setValue("unit", "");
  ...
  }, [goods_code, setValue, basicData?.goods_code]);
        ...
        <Paper sx={{ maxWidth: "800px", mx: "auto", p: 3 }}>
          ...
          >
            <Typography
              ...
              mb={0}
            >
              {getControlLabel("ttl_d_edit", "Edit AC_CONT_D")}
            </Typography>
          ...
          <Box component="form" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Grid container spacing={2} mb={3}>
              {renderField("used_qty", "Used Qty", 4, {disabled: true}, "number")}
              {renderField("stock_qty", "Stock Qty", 4, {disabled: true}, "number")}
            ...
            <Grid container spacing={2} mb={3}>
              {renderField("cont_qty", "Cont Qty", 4, {
                inputProps:{
                step: "0.0001",
                min: 0,
              ...
              <Grid item xs={4}>
                <TextField
                  fullWidth
                  label={getColumnLabel("cont_price", "Cont Price")}
                  InputLabelProps={{ shrink: true }}
                  {...register("cont_price")}
                  disabled
              ...
              {renderField("cont_money", "Cont Money", 4, {
                inputProps:{
                step: "0.0001",
                min: 0,
                onChange:handleDecimalInput(4),
