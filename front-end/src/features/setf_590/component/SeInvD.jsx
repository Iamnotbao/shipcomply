...
const SeInvD = ({
  ...
  useEffect(() => {
    const handleDataFetch = async () => {
      if (
        !parentSelectRows ||
        parentSelectRows.length === 0 ||
        !selectRows ||
        selectRows.length === 0 ||
        !parentSelectRows[0]?.factory_code ||
        !parentSelectRows[0]?.ac_no ||
        !parentSelectRows[0]?.invoice_id ||
        !selectRows[0]?.se_id ||
        !selectRows[0]?.se_seq ||
        !selectRows[0]?.se_ver
      ) {
        setData([{ tableName: "SE_INV_D", data: [] }]);
        setSelectVwContUse([]);
        return;
      }
      setCurrentVCUPage(0);
