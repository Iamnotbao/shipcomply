...
import { fetchChgM, searchChgMByFilter } from "../../../service/chg_m/chgM";

const CustomDeclarationPopup = ({
  openLink = false,
  onClose,
  data,
  setData,
  ...
  const handleSearch = async (newFilter, pageSize = 10, offset = 0) => {
    try {
      ...
      const response = await searchChgMByFilter(
        newFilter,
        user.access_token,
        user.factory,
        user.department,
      ...
      if (response && response.data && response.data.length > 0) {
        setData([{ tableName: "CHG_M", data: response.data }]);
        ...
        <DialogContent>
          <Box>
            ...
            >
              ...
              >
                {getControlLabel(
                  "ttl_customs_declaration",
                  "Select Customs Declaration",
                )}
              </Typography>
...
export default CustomDeclarationPopup;
