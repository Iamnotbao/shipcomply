...
import { createCoid } from "../../../service/ac_co_m/acCoM";
...
const AddAcCoM = ({
  ...
  const loadCoId = async () => {
    try {
      const result = await createCoid(
        user?.factory,
        user?.department,
        user?.user_code,
        auth?.find((item) => item.field === "query_level")?.title,
      );
      const co_id = result?.data || "";
      setValue("co_id", co_id);
    } catch (error) {
      console.error("Error loading CO_ID:", error);
    }
  };
        ...
        <Paper sx={{ maxWidth: "1600px", mx: "auto", p: 3 }}>
          ...
          >
            <Typography
              ...
              mb={0}
            >
              {getControlLabel("ttl_add", "Add AC_CO_M")}
            </Typography>
          ...
          <Box component="form" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Grid container spacing={2} mb={2}>
              ...
              {renderField("se_seq", "訂單序號", 2.4, {}, "number")}
