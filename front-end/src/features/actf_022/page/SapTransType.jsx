...
import { fetchAllSTT } from "../../../service/sap_trans_type/sapTransType";
const SanTransType = ({
  data,
  setData,
  selectRows,
  ...
  const fetchAll = async (authData = null, pageSize = 5, offset = 0) => {
    ...
    setData([{ tableName: "SAP_TRANS_TYPE", data: combinedData[0].data }]);
    ...
    try {
      ...
      if (response && response.data) {
        if (response.data.length === 0) {
          ...
          setData([{ tableName: "SAP_TRANS_TYPE", data: response.data }]);
        }
      }
    } catch (error) {
  ...
  const handlePageChange = async (newPage, newPageSize) => {
    ...
    const responseData = await fetchAllSTT(
      user?.factory,
      user?.department,
      user?.user_code,
