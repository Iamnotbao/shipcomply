...
import { fetchMMItemsByItemNo } from "../../../service/mm_item/MmItemService";
import { importExcel } from "../../../service/se_plan_ord/sePlanOrd";
import {
  showErrorToast,
  showSuccessToast,
  showWarningToast,
} from "../../../utils/notification/Notification";
import Pp026ExcelPopup from "./Pp026ExcelPopup";
import { exportExcel } from "../../../service/ac_co_m/acCoM";
...
const RdTemp = ({
  ...
  const fetchAll = async (authData = null, pageSize = 5, offset = 0) => {
    ...
    const combinedData = await fnQuery([
      () => getTempTable(user?.access_token, pageSize, offset),
    ]);
    console.log("comommo", combinedData[0]?.data);

    setData([{ tableName: "RD_TEMP", data: combinedData[0].data }]);
  ...
  const recreateRdTemp = async () => {
    await recreateTempTable(user?.access_token, user?.factory, user?.user_code);
    const aduangsang = await getTempTable(user?.access_token);
    console.log("adung anhdung", aduangsang);
  };
  useEffect(() => {
    recreateRdTemp();
  }, []);
    ...
    try {
      ...
      if (response && response.data) {
        if (response.data.length === 0) {
          ...
          setData([{ tableName: "RD_TEMP", data: response.data }]);
        }
      }
    } catch (error) {
  ...
  const handleImport = async (file) => {
    ...
    try {
      ...
      if (result && result.success) {
        showSuccessToast(getControlLabel, "noti_success_import");
        const response = await getTempTextTable(user.access_token, 5, 0);
        setTextImporData([
          {
            table: "TEXT_IMPORT",
            data: response?.data,
        ...
        setSelectTI([response?.data[0]]);
        setJumpToRowTI(response?.data[0]);
        setHasMoreTI(response?.hasMore);
      }
    } catch (error) {
  ...
  const handleClearImport = async () => {
    try {
      const result = await clearTempTextTable(user?.access_token);
      console.log("tt: ", result);
      if (result && result.success) {
        showSuccessToast(getControlLabel, "noti_success_clear_import");
        const response = await getTempTextTable(user.access_token, 5, 0);
        setTextImporData([
          {
            table: "TEXT_IMPORT",
  ...
  const handleExcelMaterialOut = async (search) => {
    const allow = authorization?.find(
      (item) => item.field === "query_level",
    )?.title;
    try {
      const result = await exportExcelMaterial(
        user?.access_token,
        user?.factory,
        user?.department,
        user?.user_code,
      ...
      showErrorToast(
        getControlLabel,
        `noti_error_excel_material`,
        "Failed to export excel material out",
      );
    }
  };
  const handleExcelMaterialEnd = async (search) => {
    const allow = authorization?.find(
      (item) => item.field === "query_level",
    )?.title;
    try {
      console.log("jj", dropdownValues);

      const result = await exportExcelEndMaterial(
        user?.access_token,
        user?.factory,
        user?.department,
        user?.user_code,
  ...
  const handleExcelShipOrder = async (search) => {
    const allow = authorization?.find(
      (item) => item.field === "query_level",
    )?.title;
    try {
      ...
      const result = await exportExcelShipOrder(
        user?.access_token,
        user?.factory,
        user?.department,
        user?.user_code,
  ...
  const handleExcelPP026 = async (search) => {
    const allow = authorization?.find(
      (item) => item.field === "query_level",
    )?.title;
    try {
      ...
      const result = await exportExcelPP026(
        user?.access_token,
        user?.factory,
        user?.department,
        user?.user_code,
  ...
  const refreshCurrentData = async () => {
    if (isSearch && searchFilter) {
      await handleSearchByFilter(searchFilter, currentPageSize, currentOffset);
      return;
    }

    const response = await getTempTable(
      user?.access_token,
      currentPageSize,
      currentOffset,
    );
    ...
    if (response?.data) {
      ...
      if (selectRows.length > 0) {
        // Tìm lại row đang select
        const foundRow = response.data.find(
          (item) => item.id === selectRows[0]?.id,
        );
        setSelectRows(
          foundRow
            ? [foundRow]
  ...
  const handlePageChange = async (newPage, newPageSize) => {
    ...
    // Gọi trực tiếp, không qua fetchAll (tránh bị reset state)
    const response = await getTempTable(
      user?.access_token,
      newPageSize,
      newOffset,
  ...
  const createFieldCallback = (field) => {
    return async (page, pageSize, searchText) => {
      try {
        const result = await fetchMMItemsByItemNo(page, pageSize, searchText);
  ...
  const getFetchData = {
    item_no: createFieldCallback("item_no"),
  };
  ...
  return (
    <>
          ...
          <Stack direction="row" flexWrap="wrap" sx={{ rowGap: 1 }}>
            ...
            >
              <DataTable
                ...
                onMaterialOut={handleExcelMaterialOut}
                onMaterialEnd={handleExcelMaterialEnd}
                onShipOrder={handleExcelShipOrder}
                onPp026Excel={onOpenPp026Excel}
      ...
      <Pp026ExcelPopup
        openLink={openPp026Excel}
        onClose={onClosePp026Excel}
        getControlLabel={getControlLabel}
        getColumnLabel={getColumnLabel}
        handleExport={handleExcelPP026}
        pp026ExcelForm={pp026ExcelForm}
        setPP026ExcelForm={setPP026ExcelForm}
        onFetchMatCode={getFetchData.item_no}
      />
    </>
  );
};
export default RdTemp;
