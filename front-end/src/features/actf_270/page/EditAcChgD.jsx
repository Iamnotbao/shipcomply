...
const EditAcChgD = ({
  ...
  useEffect(() => {
    const calculatedMoney = (qty || 0) * (price || 0);
  ...
  useEffect(() => {
    ...
    const calculatedTax = Math.round(
      (currRate * (money || 0) * (tax_rate || 1)) / 100,
    );
            ...
            render={({ field }) => (
              <Dropdown
                ...
                onSelect={(selectedItem) => {
                  ...
                  setDropdownValues((prev) => ({ ...prev, ac_itemno: value }));
                }}
                select={field.value || dropdownValues.ac_itemno || ""}
                table="AC_CONT_D"
                option="goods_code"
          ...
          <Controller
            ...
            render={({ field }) => (
              <Dropdown
                ...
                onSelect={(selectedItem) => {
                  const displayValue = selectedItem
                    ? `${selectedItem.cont_no || ""}-${selectedItem.seq || ""}-${parseFloat(selectedItem.cont_price || 0).toFixed(2)}`
                    : "";
                  const saveValue = parseFloat(selectedItem?.cont_price || 0);
                  field.onChange(saveValue);
                  setDropdownValues((prev) => ({
                    ...prev,
                    price: displayValue,
                    price_save: saveValue,
                  }));
                }}
                select={dropdownValues.price || ""}
                table="AC_CONT_D"
                option="cont_price"
                getControlLabel={getControlLabel}
                language={user?.language || "en"}
                ...
                totalItems={0}
                pageSize={10}
                disabled={!ac_itemno || !ac_chg_m?.min_cont}
                helperText={
                  !ac_itemno
                    ? "Please select AC Item No first"
                    : !ac_chg_m?.min_cont
                      ? "Min Cont is required"
                      : ""
                }
                {...extraProps}
              />
        ...
        <Paper sx={{ maxWidth: "1400px", mx: "auto", p: 3 }}>
          ...
          <Box component="form" onSubmit={handleSubmit(onSubmit)}>
            ...
            <Grid container spacing={2} mb={2}>
              ...
              {renderField("seq", "Seq", 3, { disabled: true }, "number")}
