...
import { addAcChgA, editAcChgA } from "../../../service/ac_chg_a/AcChgA";
import {
  showErrorToast,
  showSuccessToast,
  showWarningToast,
} from "../../../utils/notification/Notification";
import AddAcExpectMatd from "../page/AddAcExpectMatd";
...
const AcExpectMatd = ({
  ...
  const refreshCurrentData = async (updatedRecord = null) => {
    if (updatedRecord) {
        ...
        prevData.map((table) => ({
          ...
          data: table.data.map((item) => {
            const isSame =
              ...
              parseInt(item.expect_id) === parseInt(updatedRecord.expect_id) &&
      ...
      setSelectAcExpectMatd([updatedRecord]);
      return;
    }
  };
  useEffect(() => {
  ...
  useEffect(() => {
    const handleDataFetch = async () => {
      if (
        ...
        setData([{ tableName: "AC_EXPECT_MATD", data: [] }]);
        setSelectAcExpectMatd([]);
        return;
      }
      ...
      if (!isSearch) {
        ...
        await fetchDataByExpectID(true, 0, 5);
      }
    };
    handleDataFetch();
  }, [
  ...
  const handleEditClose = async (chooseRow) => {
    try {
      if (selectAcExpectMatd.length === 1) {
        const record = chooseRow || selectAcExpectMatd[0];
  ...
  const handleOpenEdit = async () => {
    try {
      ...
      const record = selectAcExpectMatd[0];
     ...
     freshRecord.locked_information !==user?.clientInfo
      ...
      setSelectAcExpectMatd([lockData]);
      setOpenEdit(true);
    } catch (error) {
      showErrorToast(
        getControlLabel,
  ...
  const handleStatusChange = async (
    ...
    if (selectAcExpectMatd.length !== 1) return;
    ...
    try {
     ...
     freshRecord.locked_information !==user?.clientInfo
        ...
        setData((prevData) =>
          prevData.map((table) => ({
            ...
            data: table.data.map((item) =>
              ...
              `${freshRecord.factory_code}-${parseInt(freshRecord.expect_id)}-${parseFloat(freshRecord.seq)}`
                ? freshRecord
                : item,
            ),
          })),
        );
        setSelectAcExpectMatd([freshRecord]);
      ...
      const response = await editAcChgA(
        user?.access_token,
        user?.factory,
        user?.department,
        user?.user_code,
      ...
      if (response.success) {
        ...
        setSelectAcExpectMatd([updateData]);
  ...
  const handleAdd = async (newData) => {
    ...
    try {
      const response = await addAcChgA(
        user?.access_token,
        user?.factory,
        user?.department,
        user?.user_code,
      ...
      if (response.success) {
        ...
        const responseData = await fetchAllAcExpectMatd(
          selectRows[0]?.factory_code,
          selectRows[0]?.expect_id,
          user.department,
          user.user_code,
        ...
        if (responseData && responseData.data) {
          ...
          setData([{ tableName: "AC_EXPECT_MATD", data: responseData.data }]);
          setSelectAcExpectMatd([response.data]);
      ...
      if (isSearch) {
        ...
        if (resultPage !== currentPage) {
          ...
          if (responseData && responseData.data) {
            ...
            if (editedRecord) {
              setSelectAcExpectMatd([editedRecord]);
  ...
  return (
    <>
      ...
      {/* <EditAcChgA
        ...
        getControlLabel={getControlLabel}
        getColumnLabel={getColumnLabel}
        selectRows={selectAcExpectMatd.length > 0 ? selectAcExpectMatd : []}
        user={user}
      /> */}
    </>
  );
};
export default AcExpectMatd;
