...
import AddAcDescProc from "../page/AddAcDescProc";
import EditAcDescProc from "../page/EditAcDescProc";
import {
  addADP,
  editADP,
  fetchAllADPByAcno,
...
const AcDescProc = ({
 ...
 const refreshCurrentData = async (updatedRecord = null) => {
  if (updatedRecord) {
    const newData = await fetchAllADPByAcno(
      user?.factory,
      selectRows[0]?.ac_no,
      user?.department,
      user?.user_code,
    ...
    if (newData?.data) {
      setData([{ tableName: "AC_DESC_PROC", data: newData.data }]);
      ...
      if (foundUpdate) setSelectADP([foundUpdate]);
    }
    return;
  }
};
  ...
  useEffect(() => {
    const handleDataFetch = async () => {
      if (
        ...
        setData([{ tableName: "AC_DESC_PROC", data: [] }]);
        setSelectADP([]);
        return;
      }
  ...
  const handleEditClose = async (chooseRow) => {
    try {
      if (selectADP.length === 1) {
        const record = chooseRow || selectADP[0];
  ...
  const handleOpenEdit = async () => {
    try {
      ...
      const record = selectADP[0];

      const freshRecord = await fetchADPRecordFromDB(record);
  ...
  const handleAdd = async (newData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const responseData = await fetchAllADPByAcno(
          selectRows[0]?.factory_code,
          selectRows[0]?.ac_no,
          user.department,
          user.user_code,
        ...
        if (responseData && responseData.data) {
          ...
          setData([{ tableName: "AC_DESC_PROC", data: responseData.data }]);
          setSelectADP([response.data]);
  ...
  return (
    <>
      ...
      <EditAcDescProc
        ...
        getControlLabel={getControlLabel}
        getColumnLabel={getColumnLabel}
        selectRows={selectADP.length > 0 ? selectADP : []}
        user={user}
        language={language}
      />
    </>
