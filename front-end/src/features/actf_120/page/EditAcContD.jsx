...
const EditAcContD = ({
  ...
  const stock_qty = watch("stock_qty");
  const used_qty = watch("used_qty");
 ...
 useEffect(() => {
    const qty = parseFloat(cont_qty) || 0;
    const u_qty = parseFloat(used_qty) || 0;
    const stock_qty = qty - u_qty;
    setValue("stock_qty", stock_qty.toFixed(4));
  }, [used_qty, cont_qty, setValue]);
  ...
  useEffect(() => {
    if (
      goods_code !== basicData?.goods_code &&
      goods_code !== dropdownValues.goods_code
    ) {
      setValue("unit", "");
