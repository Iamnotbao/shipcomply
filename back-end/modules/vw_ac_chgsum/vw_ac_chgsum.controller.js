const vwAcChgSumService = require("./vw_ac_chgsum.service");
const fs = require("fs");

async function getCont(req, res) {
  const { factory_code, department_code, user_code, query_level } = req.query;
  const result = await vwAcChgSumService.getCont(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getAllVwAcChgsum(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const result = await vwAcChgSumService.getAllVwAcChgsum(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function fetchFieldDropdown(req, res) {
  try {
    ...
    const result = await vwAcChgSumService.fetchFieldDropdown(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function fetchInAcno(req, res) {
  try {
    const { factory_code, src, out_dtype, matd_no, page, limit, search } =
      req.query;
    const result = await vwAcChgSumService.fetchInAcno(
      factory_code,
      src,
      out_dtype,
      matd_no,
...
async function fetchInCont(req, res) {
  try {
    ...
    const result = await vwAcChgSumService.fetchInCont(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function copyCont(req, res) {
  ...
  try {
    const result = await vwAcChgSumService.copyCont(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function extendCont(req, res) {
  ...
  try {
    const result = await vwAcChgSumService.extendCont(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function confirmExtendCont(req, res) {
  const { factory_code, department_code, user_code, query_level, cont_no } =
    req.query;
  try {
    const result = await vwAcChgSumService.confirmExtendCont(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function searchVwAcChgSum(req, res) {
  ...
  try {
    const shoes = await vwAcChgSumService.searchVwAcChgSum(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function exportExcelVwAcCI(req, res) {
  try {
    ...
    const { factory_code, department_code, user_code, query_level, lanaguage } =
      req.query;
    const { filters } = req.body;
    const workbook = await vwAcChgSumService.exportExcelVwAcCI(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportExcelOutVwAcChgsum(req, res) {
  try {
    const filename = "VW_AC_CHGSUM_OUT.xlsx";
    const { factory_code, department_code, user_code, query_level, lanaguage } =
      req.query;
    const { filters } = req.body;
    const workbook = await vwAcChgSumService.exportExcelOutVwAcChgsum(
      filename,
      factory_code,
      department_code,
      user_code,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=VW_AC_CHGSUM_OUT_${Date.now()}.xlsx`,
    );
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await vwAcChgSumService.exportExcelMaterialABM(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await vwAcChgSumService.exportExcelCustomABM(
      filename,
      filters,
    );
...
async function verifyRemain(req, res) {
  const { factory_code, ac_no, ac_itemno, language } = req.query;
  try {
    const result = await vwAcChgSumService.verifyRemain(
      factory_code,
      ac_no,
      ac_itemno,
      language,
...
async function updateQty(req, res) {
  const { factory_code, ac_no, ac_itemno, n_mqty, n_src } = req.query;
  try {
    const result = await vwAcChgSumService.updateQty(
      factory_code,
      ac_no,
      ac_itemno,
      n_mqty,
...
async function restoreS(req, res) {
  const { factory_code, ac_no, src } = req.query;
  try {
    const result = await vwAcChgSumService.restoreS(factory_code, ac_no, src);
    return res.json({
      message: result.message,
      success: result.success,
      tableName: "VW_AC_CHGSUM ",
