...
const createAcInMMSchema = require("./ac_inm_m.create.dto");
const acInmMService = require("./ac_inm_m.service");
...
async function getAllAcInmM(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const result = await acInmMService.getAllAcInmM(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function confirmAll(req, res) {
  const { factory_code, department_code, user_code, query_level, inm_no } =
    req.query;
  let result;
  try {
    result = await acInmMService.confirmAll(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function getAcInmMByID(req, res) {
  const { factory_code, inm_no } = req.query;
  const result = await acInmMService.getAcInmMByID(factory_code, inm_no);
  if (!result) {
...
async function addAcInmM(req, res) {
  const { page_size, factory_code, department_code, user_code, query_level } =
    req.query;
  const { data } = req.body;
  const { error, value } = createAcInMMSchema.validate(data);
  if (error) {
  ...
  try {
    const response = await acInmMService.addAcInmM(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editAcInmM(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const { error, value } = createAcInMMSchema.validate(dataToValidate);
  if (error) {
  ...
  const t = await sequelize.transaction();
  try {
    const response = await acInmMService.editAcInmM(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteAcImp(req, res) {
  ...
  try {
    ...
    const isDelete = await acInmMService.deleteAcImp(
      factory_code,
      invoice_code,
      sort,
      t,
...
async function searchAcInmM(req, res) {
  ...
  try {
    const shoes = await acInmMService.searchAcInmM(
      search,
      factory_code,
      department_code,
      user_code,
...
async function exportPDF(req, res) {
  try {
    ...
    const pdf = await acInmMService.exportPDF(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportExcelAcImp(req, res) {
  try {
    const filename = "AC_INM_M";
    const { factory_code, department_code, user_code, query_level } = req.query;
    const workbook = await acInmMService.exportExcelAcImp(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await acInmMService.exportExcelMaterialAcImp(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await acInmMService.exportExcelCustomAcImp(
      filename,
      filters,
    );
