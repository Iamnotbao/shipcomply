...
const { createAcExpectSESchema } = require("./ac_expect_se.create.dto");
const acExpectSeService = require("./ac_expect_se.service");
const sequelize = require("../../config/db");

// Helper function để parse lock_info
async function getAllAcExpectSe(req, res) {
  const {
    factory_code,
    expect_id,
    department_code,
  ...
  const result = await acExpectSeService.getAllAcExpectSe(
    factory_code,
    expect_id,
    department_code,
    user_code,
...
async function getFieldDropdown(req, res) {
  try {
    ...
    const result = await acExpectSeService.getFieldDropdown(
      factory_code,
      field,
      page,
      limit,
...
async function generateExpectId(req, res) {
  ...
  const result = await acExpectSeService.generateExpectId(factory_code);
  return res.status(200).json({
    message: "ok",
    success: true,
    data: result,
...
async function getAcExpectMByID(req, res) {
  ...
  const result = await acExpectSeService.getAcExpectMByID(
    factory_code,
    expect_id,
  );
  if (!result) {
...
async function addAcExpectM(req, res) {
  const { page_size, factory_code, department_code, user_code, query_level } =
    req.query;
  const { data } = req.body;
  const { error, value } = createAcExpectSESchema.validate(data);
  if (error) {
  ...
  try {
    const response = await acExpectSeService.addAcExpectM(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editAcExpectM(req, res) {
  ...
  const { error, value } = createAcExpectSESchema.validate(dataToValidate);
  if (error) {
  ...
  const t = await sequelize.transaction();
  try {
    const response = await acExpectSeService.editAcExpectM(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteAcImp(req, res) {
  ...
  try {
    ...
    const isDelete = await acExpectSeService.deleteAcImp(
      factory_code,
      invoice_code,
      sort,
      t,
...
async function searchAcExpectM(req, res) {
  ...
  try {
    const shoes = await acExpectSeService.searchAcExpectM(
      search,
      factory_code,
      department_code,
      user_code,
...
async function exportExcelShoeM(req, res) {
  try {
    ...
    const workbook = await acExpectSeService.exportExcelShoeM(
      filename,
      factory_code,
      department_code,
      user_code,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=AC_EXPECT_SE_${Date.now()}.xlsx`,
    );
...
async function exportExcelWriteoff(req, res) {
  try {
    ...
    const workbook = await acExpectSeService.exportExcelWriteoff(
      filename,
      factory_code,
      department_code,
      user_code,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=AC_EXPECT_SE_1_${Date.now()}.xlsx`,
    );
...
async function genOrderMaterial(req, res) {
  ...
  try {
    const result = await acExpectSeService.genOrderMaterial(
      factory_code,
      expect_id,
      user_code,
    );
...
async function calculateWriteoff(req, res) {
  const { factory_code, expect_id, user_code } = req.query;
  try {
    const result = await acExpectSeService.calculateWriteoff(
      factory_code,
      expect_id,
      user_code,
    );
