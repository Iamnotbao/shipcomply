...
const sePayService = require("./se_pay.repository");
...
async function getAllSP(
  ...
  return await sePayService.getListOfSP(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getSPByID(factory_code, pay_no) {
  return await sePayService.getByID(factory_code, pay_no);
}
async function getFieldDropdown(factory_code, language, page, limit, search) {
  return await sePayService.fetchFieldDropdown(
    factory_code,
    language,
    page,
    limit,
...
async function addSP(
  ...
  try {
    ...
    const result = await sePayService.add(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editSP(
  ...
  try {
    ...
    const result = await sePayService.edit(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteABM(factory_code, prod_acno, item_acno, t) {
  try {
    ...
    const result = await sePayService.deleteABM(existABM, t);
    if (!result) {
...
async function searchABM(
  ...
  try {
    const acABMFound = await sePayService.search(
      keyword,
      factory_code,
      department_code,
      user_code,
