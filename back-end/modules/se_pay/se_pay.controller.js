const sePayService = require("./se_pay.service");
const sequelize = require("../../config/db");
const fs = require("fs");
const createAcBomMSchema = require("./se_pay.create.dto");

async function getAllSP(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const result = await sePayService.getAllSP(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getSPByID(req, res) {
  ...
  if (!result) {
    return res.status(401).json({
      message: "Cannot get single SE_PAY!",
      success: false,
      tableName: "SE_PAY",
    });
  }
  return res.status(200).json({
    message: "Get single  SE_PAY successfully!",
    success: true,
    data: result,
    tableName: "SE_PAY",
  });
...
async function addSP(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Add  SE_PAY successfully!",
      data: response.data,
      size: response.size,
      page: response.page,
    ...
    console.log("Cannot add  SE_PAY because", error);
    await t.rollback();
  }
}
async function editSP(req, res) {
  ...
  try {
    ...
    if (!response) {
      ...
      return res.status(401).json({
        success: false,
        message: "Cannot edit  SE_PAY",
      });
    }
    await t.commit();
    return res.status(200).json({
      success: true,
      message: "Edit  SE_PAY successfully!",
      data: response.data,
      size: response.size,
      offset: response.offset,
      position: response.position,
...
async function deleteABM(req, res) {
  try {
    ...
    return res.status(200).json({
      success: true,
      message: "Delete SE_PAY successfully!",
    });
  } catch (error) {
    await t.rollback();
...
async function searchABM(req, res) {
  ...
  try {
    ...
    return res.json({
      message: "search SE_PAY successfully!",
      success: true,
      data: shoes.rows,
      total: shoes.count,
