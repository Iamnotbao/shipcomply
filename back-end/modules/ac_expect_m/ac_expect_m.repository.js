...
const AC_EXPECT_M = require("./ac_expect_m.model.js");

async function listAllAcExpectM(
  factory_code,
  department_code,
  user_code,
  query_level,
...
async function getByID(factory_code, expect_id) {
  const acImp = await AC_EXPECT_M.findOne({
    where: {
...
async function add(
  ...
  try {
    const addItem = await AC_EXPECT_M.create(acChgM, {
      transaction: t,
    });
    const permission = {
      factory_code,
...
async function generateOrderMaterial(factory_code, expect_id, user_code) {
  ...
  try {
    // 1. Xóa dữ liệu cũ trong AC_EXPECT_MATD
    await pool.query(
