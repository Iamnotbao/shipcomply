  ...
  try {
    const sql = `
      ...
      WHERE 
        ...
        AND a.status = 7
        AND (a.col7 = '7' OR a.col7 IS NULL)
        AND (b.se_custid ILIKE :se_custid OR :se_custid IS NULL)
        AND (b.se_id ILIKE :se_id OR :se_id IS NULL)
...
async function confirmPlanOrd(
  ...
  try {
    ...
    await gf_ac_pack(factory_code, ac_no, allItems,transaction);
...
async function gf_ac_pack(factory_code, ac_no, tempTable, transaction) {
  try {
    //p loop tmp_ac_plan
   ...
   await rebuildAcChgDFromPlanSize(factory_code, ac_no, transaction);
  } catch (error) {
    console.error("Error in gf_ac_pack:", error);
    throw error;
  }
}
async function deletePOrd(
  factory_code,
  ac_no,
  se_id,
  se_seq,
  ...
  if (String(status1) !== "1") {
    return { action: "DELETE_REJECTED", message: "No Permission!" };
  }

  const t = await pool.transaction();
  resetCol7Value =
    resetCol7Value === undefined ||
    resetCol7Value === null ||
    String(resetCol7Value).trim().toLowerCase() === "null"
      ? null
      : resetCol7Value;
  try {
    // 0. Kiểm tra record có tồn tại không
    const existing = await pool.query(
      `
      SELECT 1 FROM "Customs".ac_plan_ord
      WHERE factory_code = :factory_code
        AND ac_no = :ac_no
        AND se_id = :se_id
        AND se_seq = :se_seq
    ...
    if (!existing || existing.length === 0) {
      ...
      return {
        success: false,
        message: gf_mesgnm("210619", charset[language]), // TODO: đổi mã message phù hợp cho "không tìm thấy record"
      };
    }

    // 1. Xoá ac_plan_size (đơn vị đóng gói chi tiết, được gf_plan_size ghi)
    await pool.query(
      `
      DELETE FROM "Customs".ac_plan_size
      WHERE factory_code = :factory_code
        AND ac_no = :ac_no
        AND se_id = :se_id
        AND se_seq = :se_seq
      ...
      `,
      {
        replacements: { factory_code, ac_no, se_id, se_seq, ship_seq, pack_gu },
        type: pool.QueryTypes.DELETE,
        transaction: t,
      },
    );

    // 2. Xoá ac_plan_pack (sizerun/ctns đã copy từ se_plan_size)
    await pool.query(
      `
      DELETE FROM "Customs".ac_plan_pack
      WHERE factory_code = :factory_code
        AND ac_no = :ac_no
        AND se_id = :se_id
        AND se_seq = :se_seq
    ...
    // 3. Xoá ac_plan_se (bảng sequence tracking)
    await pool.query(
      `
    ...
    // 4. Xoá ac_plan_ord (bảng chính)
    await pool.query(
      `
      DELETE FROM "Customs".ac_plan_ord
      WHERE factory_code = :factory_code
        AND ac_no = :ac_no
        AND se_id = :se_id
        AND se_seq = :se_seq
    ...
    // 5. Reset lại col7 trên se_plan_ord (cho phép plan lại)
    await pool.query(
      `
      UPDATE "Customs".se_plan_ord
      SET col7 = :resetCol7Value
      WHERE factory_code = :factory_code
        AND se_id = :se_id
        AND se_seq = :se_seq
        AND ship_seq = :ship_seq
    ...
    // 6. Rebuild lại ac_chg_d (IS_REF='Y') từ ac_plan_size còn lại của AC_NO
    //    Giống hệt logic cuối gf_ac_pack — reset rồi cộng lại từ đầu để
    //    tránh lệch số do trừ thủ công.
    await rebuildAcChgDFromPlanSize(factory_code, ac_no, t);

    // 7. Tính lại PEICE / GROSS cho vw_chg_exp (giống bước 8 confirmPlanOrd)
    await pool.query(
      `
      UPDATE "Customs".vw_chg_exp t
      SET (peice, gross) = (
    ...
    // 8. Tính lại SUM_QTY / SUM_MONEY / TAX (giống bước 9 confirmPlanOrd)
    const sumResult = await pool.query(
      `
      SELECT
        SUM(COALESCE(qty, 0)) AS v_sumqty,
    ...
    return {
      success: true,
      message: "Plan order deleted and AC totals recalculated successfully",
      ac_no,
    };
  } catch (error) {
    await t.rollback();
    console.error("ERROR in deletePlanOrd:", error);
    throw error;
  }
}

/**
 * Reset QTY/MONEY của các dòng ac_chg_d do plan sinh ra (IS_REF='Y') rồi
 * cộng lại từ ac_plan_size hiện còn của AC_NO. Tách riêng để tái sử dụng
 * cho cả gf_ac_pack (sau này có thể refactor gf_ac_pack gọi lại hàm này)
 * và deletePlanOrd.
 */
async function rebuildAcChgDFromPlanSize(factory_code, ac_no, transaction) {
  await pool.query(
    `DELETE FROM "Customs".AC_CHG_D
     WHERE FACTORY_CODE = :factory_code AND AC_NO = :ac_no AND IS_REF = 'Y';`,
    { replacements: { factory_code, ac_no }, type: pool.QueryTypes.DELETE, transaction },
  );
  ...
  const acPlansizeItems = await pool.query(
    ...
    `,
    { replacements: { factory_code, ac_no }, type: pool.QueryTypes.SELECT, transaction },
  );

  // Đã DELETE sạch ở trên → không cần check tồn tại nữa, luôn INSERT.
  // acPlansizeItems rỗng (xoá hết, kể cả item cuối) → loop không chạy →
  // AC_CHG_D còn đúng 0 dòng, không sót rác.
  for (const item of acPlansizeItems) {
    const maxSeqResult = await pool.query(
      `SELECT COALESCE(MAX(SEQ),0)+1 as max_seq FROM "Customs".AC_CHG_D WHERE FACTORY_CODE = :factory_code AND AC_NO = :ac_no;`,
      { replacements: { factory_code, ac_no }, type: pool.QueryTypes.SELECT, transaction },
    );
    const y = parseInt(maxSeqResult[0]?.max_seq) || 1;
    ...
    const v_unit = await pool.query(
      ...
      { replacements: { factory_code, prod_acno: item.prod_acno || "1" }, type: pool.QueryTypes.SELECT, transaction },
    );
    ...
    await pool.query(
      `INSERT INTO "Customs".AC_CHG_D(FACTORY_CODE, AC_NO, SEQ, AC_ITEMNO, SHOE_ID, UNIT, PRICE, QTY, MONEY, TAX, TAX_RATE, IS_REF)
       VALUES (:factory_code, :ac_no, :seq, :prod_acno, :shoe_id, :unit, :price, :qty, ROUND(COALESCE(:price,0)*COALESCE(:qty,0),2), 0, 0, 'Y');`,
      {
        replacements: {
          factory_code, ac_no,
...
async function gf_mesgnm(code, language) {
  try {
    ...
    console.error("Error in gf_mesgnm:", error);
    throw error;
  }
}
module.exports = {
