...
async function getListOfAcChgM(
  factory_code,
  department_code,
  user_code,
  query_level,
  ...
  try {
    ...
    console.error("Error fetching AC_CHG_M list:", error);
    throw error;
  }
}
...
async function search(
  ...
  try {
    ...
    const sql = `
      ...
      WHERE m.ac_type::text = '1'::text
        AND ${permissionCondition}
        AND (COALESCE(v.ac_no,'') ILIKE '%'||  :ac_no || '%' )
        AND (COALESCE(v.cont_no,'') ILIKE '%'|| :cont_no || '%' )
        ...
        AND (COALESCE(v.com_invoice,'') ILIKE '%'|| :com_invoice || '%')
       AND (
  :vend_no = ''
  OR EXISTS (
    SELECT 1 
    FROM "Customs".vw_cont_imp ci
    WHERE ci.factory_code = m.factory_code
      AND ci.cont_no = v.cont_no
      AND ci.vend_no ILIKE '%'|| :vend_no || '%'
  )
)
      ORDER BY v.out_date DESC, m.ac_no
      LIMIT :limit
    ...
    const countSql = `
      ...
      INNER JOIN "Customs".vw_chg_imp v 
       ...
       WHERE m.ac_type::text = '1'::text
        ...
        AND(:e_date_1 IS NULL OR DATE_TRUNC('day', v.out_date) <= DATE_TRUNC('day', :e_date_1::timestamp)) 
        AND (v.status = :status OR :status IS NULL)
        AND (v.com_invoice ILIKE '%'|| :com_invoice || '%')
        AND (m.factory_code, v.cont_no) IN (
          SELECT factory_code, cont_no 
          FROM "Customs".vw_cont_imp
          WHERE (COALESCE(vend_no,'') ILIKE '%'|| :vend_no || '%' )
        )
    `;
