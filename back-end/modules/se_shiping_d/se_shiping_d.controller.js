// SE_SHIPING_D.controller.js
const { createSeShippingDSchema } = require("./se_shiping_d.create.dto");
const seShipingDService = require("./se_shiping_d.service");
const sequelize = require("../../config/db");

async function getAllSeShippingD(req, res) {
  const {
    factory_code,
    cust_id,
    si_seq,
  ...
  const result = await seShipingDService.getAllSeShippingD(
    factory_code,
    cust_id,
    si_seq,
    department_code,
...
async function getSeShipingDByID(req, res) {
  ...
  const result = await seShipingDService.getSeShipingDByID(
    factory_code,
    cust_id,
    si_seq,
    si_type,
...
async function updateStatusAID(req, res) {
  ...
  const { data } = req.body;
  try {
    const acItemRef = await seShipingDService.updateStatusAID(
      factory_code,
      inm_no,
      department_code,
      user_code,
...
async function addSSD(req, res) {
  ...
  try {
    const response = await seShipingDService.addSSD(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editSSD(req, res) {
  ...
  try {
    ...
    const response = await seShipingDService.editSSD(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function searchAcInmD(req, res) {
  ...
  try {
    const result = await seShipingDService.searchAcInmD(
      keyword,
      factory_code,
      department_code,
      user_code,
