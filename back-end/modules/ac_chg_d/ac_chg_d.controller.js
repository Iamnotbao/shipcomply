...
const { createAcChgDSchema } = require("./ac_chg_d.create.dto");
const acChgDService = require("./ac_chg_d.service");
...
async function getAllAcCD(req, res) {
  ...
  const result = await acChgDService.getAllAcCD(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function refreshS(req, res) {
  const { factory_code, ac_no } = req.query;
  const result = await acChgDService.refreshS(factory_code, ac_no);
  return res.status(200).json({
...
async function refreshP(req, res) {
  const { factory_code, ac_no,language } = req.query;
  const result = await acChgDService.refreshP(factory_code, ac_no,language);
  return res.status(200).json({
    message: result.message,
    success: result.success,
    tableName: "AC_CHG_D",
...
async function getAcChgDByID(req, res) {
  const { factory_code, ac_no, seq } = req.query;
  const result = await acChgDService.getAcChgDByID(factory_code, ac_no, seq);
  if (!result) {
...
async function autoAddAcChgD(req, res) {
  const { factory_code, department_code, user_code, ac_no,language } = req.query;
  try {
    const response = await acChgDService.autoAddAcChgD(
      factory_code,
      department_code,
      user_code,
      ac_no,
...
async function addAcChgD(req, res) {
  const { page_size, factory_code, department_code, user_code, query_level } =
    req.query;
  const { data } = req.body;

  const { error, value } = createAcChgDSchema.validate(data);
  if (error) {
  ...
  try {
    const response = await acChgDService.addAcChgD(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editAcChgD(req, res) {
  const t = await sequelize.transaction();
  try {
    const {
      factory_code,
    ...
    const { error, value } = createAcChgDSchema.validate(dataToValidate);
    if (error) {
      await t.rollback();
      return res.status(402).json({
        success: false,
    ...
    const response = await acChgDService.editAcChgD(
      factory_code,
      department_code,
      user_code,
      query_level,
