...
const AC_EXPECT_MATD = require("./ac_expect_matd.model.js");

async function listAllAcExpectMatD(
  factory_code,
  expect_id,
  department_code,
  user_code,
  ...
  try {
    ...
    const sql = `
     ...
     FACTORY_CODE,
        ...
        SEQ,
      MATD_NO,
      "Customs".GF_AC_ITEMNAME(FACTORY_CODE,MATD_NO,:p_charset) AS MATD_NAME, 
      "Customs".GF_CODE_NAME(FACTORY_CODE,'1105',("Customs".GF_AC_ITEMUNIT(FACTORY_CODE,MATD_NO)),:p_charset) as MATD_unitnm,
      LOSS_PER,
      EXPECT_QTY, 
      LEFT_QTY  
      FROM "Customs".AC_EXPECT_MATD 
      WHERE ${permissionCondition}
      AND EXPECT_ID =:expect_id
      ORDER BY expect_id ASC
      LIMIT :limit
      OFFSET :offset
    `;
...
async function getByID(factory_code, expect_id) {
  const acImp = await AC_EXPECT_MATD.findOne({
    where: {
...
async function add(
  ...
  try {
    const addItem = await AC_EXPECT_MATD.create(acChgM, {
      transaction: t,
    });
    const permission = {
      factory_code,
...
async function generateOrderMaterial(factory_code, expect_id, user_code) {
  ...
  try {
    // 1. Xóa dữ liệu cũ trong AC_EXPECT_MATDATD
    await pool.query(
