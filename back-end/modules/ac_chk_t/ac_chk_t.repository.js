...
const AC_CHK_T = require("./ac_chk_t.model.js");

async function listOfAcChkT(
  factory_code,
  department_code,
  user_code,
  query_level,
  ...
  try {
    ...
    const sql = `
      ...
      T.matd_seq,
        T.issue_seq,
        CASE T.src
          WHEN '1' THEN '1-OverSea IMP'
          WHEN '2' THEN '2-Invoice'
          WHEN '9' THEN '9-Process IMP'
          WHEN '0' THEN '0-Other'
        END AS src,
         CASE T.status
          WHEN 1  THEN '1-New新單'
          ...
          WHEN 9  THEN '9-Close結單'
        END AS status_name,
        T.in_acno,
        CASE T.src
          WHEN '0' THEN 'Mua npl VN'
          WHEN '2' THEN 'invoice'
          WHEN '1' THEN "Customs".GF_CHGID_CHGNO(T.factory_code, T.in_acno)
          ELSE (
            SELECT CASE mark
              WHEN 'B' THEN com_invoice
              ELSE "Customs".GF_MVCHGID_CHGNO(T.factory_code, T.in_acno)
            END
      ...
      FROM "Customs".ac_chk_t T
      WHERE
        ${permissionCondition}
        AND T.conf_seq = :conf_seq
        AND T.matd_seq = :matd_seq
      ORDER BY T.issue_seq ASC
      LIMIT :limit
      OFFSET :offset
    `;
    ...
    console.error("Error fetching AC_CHK_T:", error);
    throw error;
  }
}
...
async function getByID(factory_code, conf_seq, matd_seq, issue_seq) {
  const acImp = await AC_CHK_T.findOne({
    where: {
      factory_code: factory_code,
      conf_seq: conf_seq,
      matd_seq: matd_seq,
...
async function autoAdd(factory_code, department_code, user_code, ac_no) {
  ...
  try {
    ...
    for (const item of reqOrderResult) {
      ...
      //  Insert into AC_CHK_T
      const insertSql = `
        INSERT INTO "Customs".AC_CHK_T (
          factory_code,
          ac_no,
          seq,
          ac_itemno,
...
async function autoAddMarkB(
  ...
  try {
    ...
    // 2. Kiểm tra đã có AC_CHK_T chưa
    const checkExistingSql = `
      SELECT COUNT(1) as count
      FROM "Customs".AC_CHK_T
      WHERE factory_code = :factory_code
    ...
    // 6. Cập nhật IN_CRATE vào AC_PROC_M (lấy từ AC_CHK_T)
    try {
      const getCrateSql = `
        SELECT in_crate
        FROM "Customs".AC_CHK_T
...
async function updateExchangeRateMarkB(
  ...
  try {
    ...
    if (!in_crate || in_crate <= 0) {
      const message = await gf_mesgnm(110013, language); // "Please input exchange rate"
      throw new Error(
    ...
    // 3. Cập nhật REF_PRICE cho các record có AC_QTY > 0
    ...
    // 4. Cập nhật IN_CRATE và MONEY cho tất cả records
    // MONEY = ROUND(RB_MONEY / IN_CRATE, 2)
    const updateProcDSql = `
      UPDATE "Customs".AC_CHK_T
      SET in_crate = :in_crate,
          money = ROUND(COALESCE(rb_money, 0) / :in_crate, 2)
      WHERE factory_code = :factory_code
        AND ac_no = :ac_no
    `;
    ...
    // 5. Cập nhật IN_CRATE vào AC_PROC_M
...
async function add(
  ...
  try {
    const addItem = await AC_CHK_T.create(acImp, {
      transaction: t,
    });
    ...
    const positionInfo = await getPosition(
      {
        ...
        conf_seq: addItem.conf_seq,
        matd_seq: addItem.matd_seq,
        issue_seq: addItem.issue_seq,
      },
      pageSize,
      AC_CHK_T,
      ["issue_seq"],
  ...
  const positionInfo = await getPosition(
    {
      ...
      conf_seq: editItem.conf_seq,
      matd_seq: editItem.matd_seq,
      issue_seq: editItem.issue_seq,
    },
    pageSize,
    AC_CHK_T,
    ["issue_seq"],
...
async function deleteACT(existAcImp, t) {
  try {
...
async function search(
  ...
  try {
    ...
    let replacements = {
      ...
      matd_seq: filters.matd_seq || null,
      issue_seq: filters.issue_seq || null,
      src: filters.src || null,
      in_acno: filters.in_acno || null,
      d_type: filters.d_type || null,
    ...
    if (user_code !== "admin") {
      if (query_level === "1" && factory_code) {
        ...
        permissionCondition =
          "grt_dept = :permission_dept AND T.factory_code = :permission_factory";
        replacements.permission_dept = department_code;
        replacements.permission_factory = factory_code;
      } else if (query_level === "3" && user_code) {
        permissionCondition = "grt_user = :permission_user";
    ...
    const sql = `
      ...
      WHERE
        ...
        (:matd_seq IS NULL OR T.matd_seq = :matd_seq) AND
        (:issue_seq IS NULL OR T.issue_seq = :issue_seq) AND
        (:src IS NULL OR T.src = :src) AND
        (:in_acno IS NULL OR T.in_acno LIKE '%' || :in_acno || '%') AND
        (:d_type IS NULL OR T.d_type = :d_type)
      ORDER BY T.issue_seq ASC
    `;
    ...
    console.error("Error searching AC_CHK_T:", error);
    throw error;
  }
}
module.exports = {
