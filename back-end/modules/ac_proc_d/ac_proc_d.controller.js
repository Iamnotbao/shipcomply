...
const { createAcProcDSchema } = require("./ac_proc_d.create.dto");
const acChgDService = require("./ac_proc_d.service");
...
async function getAllAPD(req, res) {
  const { factory_code, department_code, user_code, query_level } = req.query;
  const result = await acChgDService.getAllAPD(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getAllAPDWithView(req, res) {
  ...
  const result = await acChgDService.getAllAPDWithView(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getAllAPDMarkBWithView(req, res) {
  ...
  const result = await acChgDService.getAllAPDMarkBWithView(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getAcProcDByID(req, res) {
  const { factory_code, ac_no, seq } = req.query;
  const result = await acChgDService.getAcProcDByID(factory_code, ac_no, seq);
  if (!result) {
...
async function getSum(req, res) {
  try {
    ...
    const result = await acChgDService.getSum(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function getUnitByGoodsCode(req, res) {
  try {
    ...
    const result = await acChgDService.getUnitByGoodsCode(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function autoAddAcProcD(req, res) {
  const { factory_code, department_code, user_code, ac_no,language } = req.query;
  try {
    const response = await acChgDService.autoAddAcProcD(
      factory_code,
      department_code,
      user_code,
      ac_no,
...
async function updateExRateMarkB(req, res) {
  ...
  try {
    const response = await acChgDService.updateExRateMarkB(
      factory_code,
      ac_no,
      in_crate,
      language,
...
async function autoAddAcProcDMarkB(req, res) {
  ...
  const t = await sequelize.transaction();
  try {
    const response = await acChgDService.autoAddAcProcDMarkB(
      factory_code,
      department_code,
      user_code,
      ac_no,
...
async function addAcProcD(req, res) {
  const { page_size, factory_code, department_code, user_code, query_level } =
    req.query;
  const { data } = req.body;

  const { error, value } = createAcProcDSchema.validate(data);
  if (error) {
  ...
  const t = await sequelize.transaction();
  try {
    const response = await acChgDService.addAcProcD(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editAcProcD(req, res) {
  const t = await sequelize.transaction();
  try {
    const {
      factory_code,
    ...
    const dataToValidate = { ...data, factory_code, ac_no, seq };
    const { error, value } = createAcProcDSchema.validate(dataToValidate);
    if (error) {
      await t.rollback();
      return res.status(402).json({
        success: false,
    ...
    const response = await acChgDService.editAcProcD(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteAcImp(req, res) {
  ...
  try {
    ...
    const isDelete = await acChgDService.deleteAcImp(
      factory_code,
      invoice_code,
      sort,
      t,
...
async function searchAcImp(req, res) {
  ...
  console.log(keyword);
  try {
    const shoes = await acChgDService.searchAcImp(
      keyword,
      factory_code,
      department_code,
      user_code,
...
async function exportExcelAcImp(req, res) {
  try {
    ...
    const { factory_code, department_code, user_code, query_level } = req.query;
    const workbook = await acChgDService.exportExcelAcImp(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await acChgDService.exportExcelMaterialAcImp(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await acChgDService.exportExcelCustomAcImp(
      filename,
      filters,
    );
