  ...
  try {
    const sql = `
      ...
      WHERE 
        ...
        AND (
          :cust_id IS NULL 
          OR "Customs".GF_CUSTID_CUSTNO(A.ORG_ID, A.SE_CUSTID) LIKE :cust_id || '%'
        )
        AND (
          :s_date IS NULL 
    ...
    for (const treeItem of selectedItems) {
      try {
        ...
        try {
          ...
          console.warn(
            " GF_SE_PLAN_SHIPQTY failed, using 0:",
            funcErr.message,
          );
          try {
  ...
  try {
    const updateSql = `
      ...
      WHERE M.org_id = T.FACTORY_CODE
        ...
        AND (T.SE_ID LIKE :se_id || '%' OR :se_id IS NULL)
        AND (T.STATUS = :status OR :status IS NULL)
        AND (T.COLUMN1 = :hg_stoc OR :hg_stoc IS NULL)
        AND (T.COL5 LIKE :agent || '%' OR :agent IS NULL)
