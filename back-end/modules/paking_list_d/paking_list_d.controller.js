const pakingListDService = require("./paking_list_d.service");
const sequelize = require("../../config/db");
const fs = require("fs");
const createAcBomMSchema = require("./paking_list_d.create.dto");

async function getAllPLD(req, res) {
  const {
   factory_code,
  department_code,
  user_code,
  ...
  const result = await pakingListDService.getAllPLD(
   factory_code,
  department_code,
  user_code,
  query_level,
...
async function getSPByID(req, res) {
  ...
  const result = await pakingListDService.getSPByID(factory_code, pay_no);
  if (!result) {
...
async function addSP(req, res) {
  ...
  try {
    const response = await pakingListDService.addSP(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editSP(req, res) {
  ...
  try {
    ...
    const response = await pakingListDService.editSP(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteABM(req, res) {
  try {
    ...
    const t = await sequelize.transaction();
    const isDelete = await pakingListDService.deleteABM(
      factory_code,
      prod_acno,
      item_acno,
      t,
...
async function searchPLM(req, res) {
  ...
  console.log(search);
  try {
    const shoes = await pakingListDService.searchPLM(
      factory_code,
      user_code,
      query_level,
      department_code,
...
async function exportPDFABM(req, res) {
  try {
    ...
    const pdf = await pakingListDService.exportPDFABM(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await pakingListDService.exportExcelMaterialABM(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await pakingListDService.exportExcelCustomABM(
      filename,
      filters,
    );
