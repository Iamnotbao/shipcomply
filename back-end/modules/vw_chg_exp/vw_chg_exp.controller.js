const vwAcChgExpService = require("./vw_chg_exp.service");
const fs = require("fs");

async function getListOfAcCE(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const result = await vwAcChgExpService.getListOfAcCE(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getCIContract(req, res) {
  ...
  const result = await vwAcChgExpService.getCIContract(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function copyCont(req, res) {
  ...
  try {
    const result = await vwAcChgExpService.copyCont(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function extendCont(req, res) {
  ...
  try {
    const result = await vwAcChgExpService.extendCont(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function confirmExtendCont(req, res) {
  const { factory_code, department_code, user_code, query_level, cont_no } =
    req.query;
  try {
    const result = await vwAcChgExpService.confirmExtendCont(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function searchVwChgExp(req, res) {
  ...
  try {
    const shoes = await vwAcChgExpService.searchVwChgExp(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function exportExcelVwAcCI(req, res) {
  try {
    ...
    const { factory_code, department_code, user_code, query_level, lanaguage } =
      req.query;
    const { filters } = req.body;
    const workbook = await vwAcChgExpService.exportExcelVwAcCI(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await vwAcChgExpService.exportExcelMaterialABM(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await vwAcChgExpService.exportExcelCustomABM(
      filename,
      filters,
    );
