...
const fs = require("fs");
const AC_CHG_A = require("./ac_chg_a.model.js");
const FACTORY = require("../factories/factory.model.js");

async function listAllAcChgA(
  factory_code,
  department_code,
  user_code,
  query_level,
...
async function getByID(factory_code, ac_no, seq) {
  const acImp = await AC_CHG_A.findOne({
    where: {
      factory_code: factory_code,
      ac_no: ac_no,
      seq: seq,
...
async function add(
  ...
  try {
    const maxSeq = await AC_CHG_A.max("seq", {
      where: {
    ...
    const addItem = await AC_CHG_A.create(
      { ...acImp, seq: nextSeq },
      {
        transaction: t,
      },
...
async function search(
  ...
  try {
    ...
    const whereClause = queryHelper.whereMap.AC_CHG_A || {};
    if (user_code !== "admin") {
      if (query_level === "1" && factory_code) {
        whereClause.factory_code = factory_code;
      } else if (query_level === "2" && department_code) {
    ...
    const impSearch = await AC_CHG_A.findAll({
      where: whereClause,
      include: [
        {
          model: FACTORY,
