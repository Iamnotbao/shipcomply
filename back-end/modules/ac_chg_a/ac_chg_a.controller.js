...
const { createAcChgASchema } = require("./ac_chg_a.create.dto");
const acChgAService = require("./ac_chg_a.service");
...
async function getAllAcCA(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const result = await acChgAService.getAllAcCA(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getAcChgAByID(req, res) {
  const { factory_code, ac_no, seq } = req.query;
  const result = await acChgAService.getAcChgAByID(factory_code, ac_no, seq);
  if (!result) {
...
async function getSum(req, res) {
  try {
    ...
    const result = await acChgAService.getSum(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function getUnitByGoodsCode(req, res) {
  try {
    ...
    const result = await acChgAService.getUnitByGoodsCode(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function addAcChgA(req, res) {
  const { page_size, factory_code, department_code, user_code, query_level } =
    req.query;
  const { data } = req.body;

  const { error, value } = createAcChgASchema.validate(data);
  if (error) {
  ...
  try {
    const response = await acChgAService.addAcChgA(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editAcChgA(req, res) {
  const t = await sequelize.transaction();
  try {
    const {
      factory_code,
    ...
    const { error, value } = createAcChgASchema.validate(dataToValidate);
    if (error) {
      await t.rollback();
      return res.status(402).json({
        success: false,
    ...
    const response = await acChgAService.editAcChgA(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteAcImp(req, res) {
  ...
  try {
    ...
    const isDelete = await acChgAService.deleteAcImp(
      factory_code,
      invoice_code,
      sort,
      t,
...
async function searchAcImp(req, res) {
  ...
  console.log(keyword);
  try {
    const shoes = await acChgAService.searchAcImp(
      keyword,
      factory_code,
      department_code,
      user_code,
...
async function exportExcelAcImp(req, res) {
  try {
    ...
    const { factory_code, department_code, user_code, query_level } = req.query;
    const workbook = await acChgAService.exportExcelAcImp(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await acChgAService.exportExcelMaterialAcImp(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await acChgAService.exportExcelCustomAcImp(
      filename,
      filters,
    );
