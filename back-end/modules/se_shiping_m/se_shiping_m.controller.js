// SE_SHIPING_M.controller.js
const createSeShipingMSchema = require("./se_shiping_m.create.dto");
const seShipingMService = require("./se_shiping_m.service");
...
async function getAllSeShipingM(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const result = await seShipingMService.getAllSeShipingM(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getSeShippingMByID(req, res) {
  ...
  const result = await seShipingMService.getSeShippingMByID(
    factory_code,
    cust_id,
    si_seq,
  );
...
async function getSiSeq(req, res) {
  const { factory_code, cust_id, department_code, user_code, query_level } =
    req.query;
  const result = await seShipingMService.getSiSeq(
    factory_code,
    cust_id,
    department_code,
    user_code,
...
async function addSeShipingM(req, res) {
  ...
  try {
    const response = await seShipingMService.addSeShipingM(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editSeShipingM(req, res) {
  ...
  const t = await sequelize.transaction();
  try {
    const response = await seShipingMService.editSeShipingM(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteAcImp(req, res) {
  ...
  try {
    ...
    const lockKey = `SE_SHIPING_M:${factory_code}:${invoice_code}:${sort}`;
    ...
    const isDelete = await seShipingMService.deleteAcImp(
      factory_code,
      invoice_code,
      sort,
      t,
...
async function searchSeShipingM(req, res) {
  ...
  try {
    const shoes = await seShipingMService.searchSeShipingM(
      search,
      factory_code,
      department_code,
      user_code,
...
async function exportPDF(req, res) {
  try {
    const filename = "SE_SHIPING_M.pdf";
    const { factory_code, department_code, user_code, query_level } = req.query;
    const pdf = await seShipingMService.exportPDF(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportExcel(req, res) {
  try {
    const filename = "SE_SHIPING_M";
    const { factory_code, department_code, user_code, query_level } = req.query;
    const { search } = req.body;
    const workbook = await seShipingMService.exportExcel(
      filename,
      search,
      factory_code,
      department_code,
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=SE_SHIPING_M_${Date.now()}.xlsx`,
    );
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await seShipingMService.exportExcelMaterialAcImp(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await seShipingMService.exportExcelCustomAcImp(
      filename,
      filters,
    );
...
async function confirmAll(req, res) {
  ...
  const result = await seShipingMService.confirmAll(
    factory_code,
    department_code,
    user_code,
    query_level,
  ...
  if (!result) {
    return res.status(401).json({
      message: "Cannot get single se shiping m!",
      success: false,
      tableName: "SE_SHIPING_M",
    });
  }
  return res.status(200).json({
    message: "Get single se shiping m successfully!",
    success: true,
    data: result,
    tableName: "SE_SHIPING_M",
  });
