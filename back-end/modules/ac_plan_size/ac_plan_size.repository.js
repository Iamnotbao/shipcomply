...
const fs = require("fs");
const AC_PLAN_SIZE = require("./ac_plan_size.model.js");
const FACTORY = require("../factories/factory.model.js");
const { Op } = require("sequelize");
const pool = require("../../config/db.js");

async function getListOfAcPLanSize(
  factory_code,
  department_code,
  user_code,
  query_level,
  ...
  try {
    ...
    console.error("Error fetching AC_PLAN_SIZE:", error);
    throw error;
  }
}
...
async function getByID(
  ...
  const acBomM = await AC_PLAN_SIZE.findOne({
    where: {
      factory_code: factory_code,
      ac_no: ac_no,
      se_id: se_id,
...
async function add(
  ...
  const addItem = await AC_PLAN_SIZE.create(acBomM, { transaction: t });
...
async function edit(
  ...
  try {
    ...
    const positionInfo = await getPosition(
      {
        se_id: editItem.se_id,
        factory_code: editItem.factory_code,
        ac_no: editItem.ac_no,
        se_seq: editItem.se_seq,
        se_ver: editItem.se_ver,
        pack_gu: editItem.pack_gu,
        ship_seq: editItem.ship_seq,
        size_no: editItem.size_no,
      },
      pageSize,
      AC_PLAN_SIZE,
      t,
...
async function search(
  ...
  try {
    const queryHelper = new QueryHelper(query, {
      AC_PLAN_SIZE: ["prod_acno", "item_acno", "status"],
      FACTORY: ["factory_code"],
    }).filter();
    const whereClause = queryHelper.whereMap.AC_PLAN_SIZE || {};
    if (user_code !== "admin") {
      if (query_level === "1" && factory_code) {
        whereClause.factory_code = factory_code;
      } else if (query_level === "2" && department_code) {
    ...
    const rows = await AC_PLAN_SIZE.findAll({
      where: whereClause,
      include: [
        {
          model: FACTORY,
    ...
    if (parseInt(offset) === 0) {
      total = await AC_PLAN_SIZE.count({
        where: whereClause,
      });
    }
    return {
...
async function updateProdAcno(
  ...
  if (user_code === "admin") {
    return await AC_PLAN_SIZE.findAll({
      order: [
        ["factory_code", "ASC"],
        ["ac_no", "ASC"],
      ],
  ...
  try {
    ...
    const sql1 = `UPDATE "Customs".ac_plan_size 
      SET prod_acno = :prod_acno, last_user = :user_code, last_date = NOW()
      WHERE 
      factory_code = :factory_code 
      AND ac_no = :ac_no
      AND se_id = :se_id 
