const vwAcContImpService = require("./vw_cont_imp.service");
const fs = require("fs");

async function getListOfCI(req, res) {
  const { factory_code, department_code, user_code, query_level, language } =
    req.query;
  const { filters } = req.body;
  const result = await vwAcContImpService.getListOfCI(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function fetchGroupFieldDataDropdown(req, res) {
  try {
    const {
      factory_code,
      department_code,
    ...
    const result = await vwAcContImpService.fetchGroupFieldDataDropdown(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function updateLastExpDate(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      data: {
        cont_no: result.cont_no,
        last_edate: result.last_edate,
        last_user: result?.user_code,
      },
      tableName: "VW_CONT_IMP_1",
    });
  } catch (error) {
