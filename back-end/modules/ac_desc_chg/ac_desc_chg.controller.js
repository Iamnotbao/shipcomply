...
const { createAcDescChgSchema } = require("./ac_desc_chg.create.dto");
const acDescChgService = require("./ac_desc_chg.service");
...
async function getAllAcDC(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const result = await acDescChgService.getAllAcDC(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getAcDescChgByID(req, res) {
  const { factory_code, ac_no, seq } = req.query;
  const result = await acDescChgService.getAcDescChgByID(factory_code, ac_no, seq);
  if (!result) {
...
async function getSum(req, res) {
  try {
    ...
    const result = await acDescChgService.getSum(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function getUnitByGoodsCode(req, res) {
  try {
    ...
    const result = await acDescChgService.getUnitByGoodsCode(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function addAcDescChg(req, res) {
  const { page_size, factory_code, department_code, user_code, query_level } =
    req.query;
  const { data } = req.body;

  const { error, value } = createAcDescChgSchema.validate(data);
  if (error) {
  ...
  try {
    const response = await acDescChgService.addAcDescChg(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editAcDescChg(req, res) {
  const t = await sequelize.transaction();
  try {
    const {
      factory_code,
    ...
    const { error, value } = createAcDescChgSchema.validate(dataToValidate);
    if (error) {
      await t.rollback();
      return res.status(402).json({
        success: false,
    ...
    const response = await acDescChgService.editAcDescChg(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteAcImp(req, res) {
  ...
  try {
    ...
    const isDelete = await acDescChgService.deleteAcImp(
      factory_code,
      invoice_code,
      sort,
      t,
...
async function searchAcImp(req, res) {
  ...
  console.log(keyword);
  try {
    const shoes = await acDescChgService.searchAcImp(
      keyword,
      factory_code,
      department_code,
      user_code,
...
async function exportExcelAcImp(req, res) {
  try {
    ...
    const { factory_code, department_code, user_code, query_level } = req.query;
    const workbook = await acDescChgService.exportExcelAcImp(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await acDescChgService.exportExcelMaterialAcImp(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await acDescChgService.exportExcelCustomAcImp(
      filename,
      filters,
    );
