const acPlanPackService = require("./ac_plan_pack.service");
const sequelize = require("../../config/db");
const fs = require("fs");
const createAcPlanPackSchema = require("./ac_plan_pack.create.dto");

async function getListOfAPP(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const result = await acPlanPackService.getListOfAPP(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getSPByID(req, res) {
  ...
  const result = await acPlanPackService.getSPByID(factory_code, pay_no);
  if (!result) {
...
async function getFieldDropdown(req, res) {
  ...
  const result = await acPlanPackService.getFieldDropdown(
    factory_code,
    language,
    page,
    limit,
...
async function addSP(req, res) {
  const { page_size, factory_code, department_code, user_code, query_level } =
    req.query;
  const { data } = req.body;
  const { error, value } = createAcPlanPackSchema.validate(data);
  if (error) {
  ...
  try {
    const response = await acPlanPackService.addSP(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editSP(req, res) {
  ...
  try {
    ...
    const { error, value } = createAcPlanPackSchema.validate(dataToValidate);
    if (error) {
      return res.status(402).json({
        success: false,
        message: error.details[0].message,
    ...
    const response = await acPlanPackService.editSP(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteABM(req, res) {
  try {
    ...
    const t = await sequelize.transaction();
    const isDelete = await acPlanPackService.deleteABM(
      factory_code,
      prod_acno,
      item_acno,
      t,
...
async function searchABM(req, res) {
  ...
  console.log(keyword);
  try {
    const shoes = await acPlanPackService.searchABM(
      keyword,
      factory_code,
      department_code,
      user_code,
...
async function exportPDFABM(req, res) {
  try {
    ...
    const pdf = await acPlanPackService.exportPDFABM(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await acPlanPackService.exportExcelMaterialABM(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await acPlanPackService.exportExcelCustomABM(
      filename,
      filters,
    );
