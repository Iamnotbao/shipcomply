const vwAcContUseService = require("./vw_cont_use.service");
const fs = require("fs");

async function getListOfVCU(req, res) {
  const { factory_code, cont_no, goods_code, language, limit, offset } =
    req.query;
  const result = await vwAcContUseService.getListOfVCU(
    factory_code,
    cont_no,
    goods_code,
    language,
...
async function searchVwAcContImp(req, res) {
  ...
  try {
    const shoes = await vwAcContUseService.searchVwAcContImp(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function exportExcelVwAcCI(req, res) {
  try {
    ...
    const workbook = await vwAcContUseService.exportExcelVwAcCI(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await vwAcContUseService.exportExcelMaterialABM(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await vwAcContUseService.exportExcelCustomABM(
      filename,
      filters,
    );
