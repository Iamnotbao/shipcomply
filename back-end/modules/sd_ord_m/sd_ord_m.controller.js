const SdOrdMService = require("./sd_ord_m.service");
const sequelize = require("../../config/db");
const fs = require("fs");
const createAcBomMSchema = require("./sd_ord_m.create.dto");

async function getAllSdOM(req, res) {
  const {
    factory_code,
    department_code,
    user_code,
  ...
  const result = await SdOrdMService.getAllSdOM(
    factory_code,
    department_code,
    user_code,
    query_level,
...
async function getSPByID(req, res) {
  ...
  const result = await SdOrdMService.getSPByID(factory_code, pay_no);
  if (!result) {
...
async function addSP(req, res) {
  ...
  try {
    const response = await SdOrdMService.addSP(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editSP(req, res) {
  ...
  try {
    ...
    const response = await SdOrdMService.editSP(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteABM(req, res) {
  try {
    ...
    const t = await sequelize.transaction();
    const isDelete = await SdOrdMService.deleteABM(
      factory_code,
      prod_acno,
      item_acno,
      t,
...
async function searchSdOM(req, res) {
  ...
  console.log(search);
  try {
    const shoes = await SdOrdMService.searchSdOM(
      factory_code,
      user_code,
      query_level,
      department_code,
...
async function exportPDFABM(req, res) {
  try {
    ...
    const pdf = await SdOrdMService.exportPDFABM(
      filename,
      factory_code,
      department_code,
      user_code,
...
async function exportMaterialToExcel(req, res) {
  try {
    ...
    const filePath = await SdOrdMService.exportExcelMaterialABM(
      filename,
      filters,
    );
...
async function exportCustomToExcel(req, res) {
  try {
    ...
    const filePath = await SdOrdMService.exportExcelCustomABM(
      filename,
      filters,
    );
