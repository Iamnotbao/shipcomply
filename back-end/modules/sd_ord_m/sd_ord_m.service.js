...
const SdOrdMRepository = require("./sd_ord_m.repository");

async function getAllSdOM(
  factory_code,
  department_code,
  user_code,
  query_level,
  ...
  return await SdOrdMRepository.getListSdOM(
    factory_code,
  department_code,
  user_code,
  query_level,
...
async function getSPByID(factory_code, pay_no) {
  return await SdOrdMRepository.getByID(factory_code, pay_no);
}
async function addSP(
  ...
  try {
    ...
    const result = await SdOrdMRepository.add(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function editSP(
  ...
  try {
    ...
    const result = await SdOrdMRepository.edit(
      factory_code,
      department_code,
      user_code,
      query_level,
...
async function deleteABM(factory_code, prod_acno, item_acno, t) {
  try {
    ...
    const result = await SdOrdMRepository.deleteABM(existABM, t);
    if (!result) {
...
async function searchSdOM(
  ...
  try {
    const acABMFound = await SdOrdMRepository.search(
       factory_code,
  user_code,
  query_level,
  department_code,
