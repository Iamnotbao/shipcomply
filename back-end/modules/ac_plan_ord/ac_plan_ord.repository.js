...
const fs = require("fs");
const AC_PLAN_ORD = require("./ac_plan_ord.model.js");
...
async function getListOfAcPLanOrd(
  factory_code,
  department_code,
  user_code,
  query_level,
  ...
  try {
    ...
    const sql = `
      ...
      t.ac_no,
          ...
          (
              ...
              FROM mxcp.SD_030
              WHERE t.SE_ID LIKE VBELN || '%'
                AND WERKS = t.factory_code
          ) AS PAY_CUST,

          (
              SELECT
                  CASE :p_charset
                      WHEN 'L' THEN name_q_s
          ...
          t.PROD_NO,
          "Customs".GF_PROD_NAME(t.factory_code, t.PROD_NO, :p_charset) AS PROD_NAME,
          t.IS_SHIP

      FROM "Customs".AC_PLAN_ORD t
    ...
    console.error("Error fetching AC_PLAN_ORD:", error);
    throw error;
  }
}
...
async function getByID(factory_code, pay_no) {
  const acBomM = await AC_PLAN_ORD.findOne({
    where: {
      factory_code: factory_code,
      pay_no: pay_no,
    },
...
async function add(
  ...
  const addItem = await AC_PLAN_ORD.create(acBomM, { transaction: t });
...
async function search(
  ...
  try {
    const queryHelper = new QueryHelper(query, {
      AC_PLAN_ORD: ["prod_acno", "item_acno", "status"],
      FACTORY: ["factory_code"],
    }).filter();
    const whereClause = queryHelper.whereMap.AC_PLAN_ORD || {};
    if (user_code !== "admin") {
      if (query_level === "1" && factory_code) {
        whereClause.factory_code = factory_code;
      } else if (query_level === "2" && department_code) {
    ...
    const rows = await AC_PLAN_ORD.findAll({
      where: whereClause,
      include: [
        {
          model: FACTORY,
