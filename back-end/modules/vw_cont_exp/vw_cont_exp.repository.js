...
async function search(
  ...
  try {
    ...
    let replacements = {
      ...
      cont_no: filters.cont_no || null,
      status: filters.status ?? null,
      s_issuedate: filters.s_date_1 || null,
      e_issuedate: filters.e_date_1 || null,
      s_expiredate: filters.s_date_2 || null,
      e_expiredate: filters.e_date_2 || null,
      cont_category: filters.cont_category || null,
    ...
    if (user_code !== "admin") {
     ...
     WHERE
        ${permissionCondition}
        AND FACTORY_CODE = :factory_code
        AND (COALESCE(CONT_NO,'') ILIKE '%' || :cont_no || '%' OR :cont_no IS NULL)
        ...
        AND (COALESCE(cont_category,'') ILIKE '%' || :cont_category || '%' OR :cont_category IS NULL  )
      ORDER BY CONT_NO
      LIMIT :limit 
      OFFSET :offset
    `;
